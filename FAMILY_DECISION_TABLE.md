...
| `twolobe_d8maplate` | repaired map-layer D8-tail runtime descendant | strongest repaired runtime-descendant candidate | On refreshed current-base maps it preserves the corrected raw `twolobe` `D60` gain exactly while repairing the residual `D21 D=8.0` tail: `D21 p95 1.78 -> 0.89`, zero acceptance flips, and no regression at `D=6.4`. | Promote only after checking that no hidden regressions appear outside the reviewer slices and deciding whether it should replace corrected raw `twolobe` as the primary reopened descendant. | `output/kinetic_action_chain/observable_two_lobe_d8_tail_guard_audit_summary.csv`, `output/kinetic_action_chain/observable_two_lobe_d8_maplate_currentbase_runtime_validation_summary.csv`, `output/hll_signal_strength/hll_signal_strength_summary_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_d8maplate_D60E21_runtime_20260329_refsnapfix_d8maplate_r3.csv`, `output/hll_signal_strength/hll_signal_strength_summary_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_d8maplate_D21E41_runtime_20260329_refsnapfix_d8maplate_r3.csv` |
...
- **Keep alive as research ingredients**:
  ...
  - `twolobe_d8maplate` as the strongest repaired runtime-descendant candidate on refreshed current-base maps,
  - Schur/logdet conditional features,
