...
| `twolobe_maplate` | map-layer late-blend descendant | `CLOSED - NEGATIVE` | On refreshed current-base maps it is numerically equivalent to `twolobe_latedstrip` and still does not beat corrected raw `twolobe`. | `output/kinetic_action_chain/observable_two_lobe_maplate_currentbase_runtime_validation_summary.csv` |
...
| recentered two-lobe D-phase notch / corrected raw `twolobe` runtime | strongest runtime-descendant reviewer candidate | A `ref_D` snap bug in the first runtime validations materially overstated the `D21` failure. After the fix, corrected raw `twolobe` is genuinely positive at reviewer level: `D60 p95 31.29 -> 18.58`, `D21 p95 2.10 -> 1.78`, with the remaining cost concentrated in the `D21 D=8.0` tail rather than the dominant `D=6.4` strip. | Promote only if a follow-up guard removes the `D21 D=8.0` / acceptance-flip cost without giving back the corrected `D60` gain. Descendants such as `latedstrip` and `maplate` are not currently better than the corrected parent family. | `output/kinetic_action_chain/observable_two_lobe_dphase_notch_recentered_audit_summary.csv`, `output/kinetic_action_chain/observable_two_lobe_runtime_mode_validation_summary.csv`, `output/hll_signal_strength/hll_signal_strength_summary_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_D60E21_runtime_20260329_refsnapfix.csv`, `output/hll_signal_strength/hll_signal_strength_summary_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_D21E41_runtime_20260329_refsnapfix.csv` |
...
- **Keep alive as research ingredients**:
  ...
  - corrected raw `twolobe` as the strongest reopened runtime-descendant candidate after the `ref_D` snap fix,
  - Schur/logdet conditional features,
