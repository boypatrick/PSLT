...
- **Primary full_direct release mode:** `code/publish_full_direct_map.py` publishes one integrated release table combining:
  ...
  Both scan scripts now expose a release production per-cell direct `g_N+kinetics` mode via `chain_mode=cell_direct_runtime`: `g_N(D)` (via `g_mode=fp_2d_full_runtime_direct`), `chi_LR(D)`, and `A_l(D)` are evaluated by direct solvers inside scan evaluation, while `B_N` remains on release EFT-operator profile closure (`b_mode=eft_operator_norm`). On the D21×E41 parity audit against `full_direct`, this mode shows bounded drift (`Δf(χ²<4)=-0.0290`, acceptance mismatch `0.0290`, `max|Δμ_μμ|=5.02`; source: `paper/chain_mode_cell_direct_audit_Dgrid21_Egrid41.csv`). The stress-only all-direct path is kept as `chain_mode=cell_direct_runtime_extreme`, where EFT-operator visibility inputs `(g_{iN}(D), λ_N(D))` are also solved per cell (`b_mode=eft_operator_norm_runtime_direct`); with `--runtime-direct-no-cache`, this becomes strict per-call recomputation (very expensive; diagnostic only).
