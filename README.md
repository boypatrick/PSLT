...
- **Primary full_direct release mode:** `code/publish_full_direct_map.py` publishes one integrated release table combining:
  ...
  - small-surface chain-mode parity audit (`full_direct` vs `cell_direct_runtime`, `D21 x E41`)
  - large-surface chain-mode parity audit (`full_direct` vs `cell_direct_runtime`, `D60 x E21`)
  - large-surface chain-mode parity audit (`full_direct` vs `cell_direct_runtime_extreme`, `D60 x E21`)
  ...
  Both scan scripts now expose a release production per-cell direct `g_N+kinetics` mode via `chain_mode=cell_direct_runtime`: `g_N(D)` (via `g_mode=fp_2d_full_runtime_direct`), `chi_LR(D)`, and `A_l(D)` are evaluated by direct solvers inside scan evaluation, while `B_N` remains on release EFT-operator profile closure (`b_mode=eft_operator_norm`). On parity audits against `full_direct`, this mode shows bounded drift at both scales: D21×E41 (`Δf(χ²<4)=-0.0290`, acceptance mismatch `0.0290`, `max|Δμ_μμ|=5.02`; source: `paper/chain_mode_cell_direct_audit_Dgrid21_Egrid41.csv`) and D60×E21 (`Δf(χ²<4)=-0.00714`, acceptance mismatch `0.00714`, `max|Δμ_μμ|=4.33`; source: `paper/chain_mode_cell_direct_audit_Dgrid60_Egrid21.csv`). The stress-only all-direct path is kept as `chain_mode=cell_direct_runtime_extreme`, where EFT-operator visibility inputs `(g_{iN}(D), λ_N(D))` are also solved per cell (`b_mode=eft_operator_norm_runtime_direct`); on D60×E21 this extreme branch drifts more strongly (`Δf(χ²<4)=-0.0444`, mismatch `0.0444`, `max|Δμ_μμ|=55.0`; source: `paper/chain_mode_cell_direct_audit_Dgrid60_Egrid21_cell_direct_runtime_extreme.csv`). With `--runtime-direct-no-cache`, this becomes strict per-call recomputation (very expensive; diagnostic only).
