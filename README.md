...
## Positioning Relative to Gauge-Unification Models
- SU(5), SO(10), Pati-Salam, E6, and SU(6)-type constructions are treated as complementary gauge-representation frameworks, not as PSLT baselines.
- Their main role is to organize SM charge assignments or family-sized representations; the number of replicated families is usually an external multiplicity.
- PSLT addresses the narrower scan-level question of first-three-layer spectral occupancy concentration under the stated A1-A5 assumptions.
- The manuscript now includes a short reviewer-facing comparison table in the Introduction; it does not open a new GUT, charge-unification, or family-count theorem line.
- The introduction also cites the standard CKM/PMNS, flavor-symmetry, and string-compactification references used to frame the family-count problem.

## First-Principles Migration Status
- The submission-freeze tracker is maintained in `/Users/boypatrick/codex/PSLT_quantam/ROADMAP_STATUS.md`, including active packaging checks, closed theorem/profile gates, and optional post-submission strengthening items.  It is the authoritative current roadmap; the detailed bullets below are provenance, not an instruction to reopen closed lines.
...
- **Static-width complex-pole gate:** The paper now states the strict upgrade as Proposition S1--S25 and adopts S25 as the reviewer-facing strict static-width promotion theorem on the audited `D=6,R_C=400,R=60->80,c=640` parent tube. `\Gamma_{N,\ell}^{(geo)}=\omega_N e^{-2S_{N,\ell}}` remains the conservative fallback/comparator outside that certified tube. S1 shows the CAP negative control is absorber-linear and local outgoing-Robin roots are not `R`-stable (`max span_R Gamma = 1.598`, `min Gamma(R=120)/Gamma(R=80)=0.463`); the combined physical extractor (`/Users/boypatrick/codex/PSLT_quantam/code/extract_static_width_physical.py`) marks all benchmark rows `not_promoted` under this initial CAP/Siegert diagnostic gate. S2 replaces local Robin by Riccati DtN but still fails (`max span_R Gamma = 1.083`, `max span_Rtail Gamma = 0.979`, `min Gamma(R=120)/Gamma(R=80)=0.716`). S3 replaces the WKB start by analytic Coulomb-tail normalization (`--tail-mode coulomb_series --coulomb-order 6 --tail-validation --fallback-guesses`), giving candidate widths `1.67e-3..1.87e-3` but still failing (`max span_R Gamma = 0.763`, `max span_Rc Gamma = 0.617`). S4/S5 close the current ECS route: roundoff residuals are achieved, but both tracked and optimistic branch continuations remain scaling-sensitive and no seed-qualified branch passes. S6 adds the full Coulomb/Whittaker DtN backend in `/Users/boypatrick/codex/PSLT_quantam/code/extract_static_width_exact_dtn.py` (`--tail-mode whittaker --whittaker-dps 50 --tail-validation --fallback-guesses`), using complex `mpmath.hyperu` for the exact Whittaker logarithmic derivative. It lowers the tail-start ambiguity but remains `not_promoted`: candidate medians are `1.66e-3..1.91e-3`, `max root residual = 9.82e-7`, `max span_R Gamma = 0.763`, `max span_Rc Gamma = 0.536`, `Gamma(R=120)/Gamma(R=80)=0.673..0.681`, and the `dps=80` spot check is identical at displayed precision. S7 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_branch_isolation.py`, an optimistic nearest-neighbor branch-isolation audit over the resolved Whittaker roots: `0/48` anchor continuations pass, with best `D=12,18` normalized stability scores still `10.53` and `10.43` and the same `R/Rc` obstruction. S8 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_argument_principle.py`: all `48/48` Whittaker roots have local argument index `-1` on a `1e-4` rectangle, `min boundary |F| = 2.40e-2`, and max Newton radius / contour radius `7.66e-7`, so they are isolated local finite-radius zeros rather than solver ghosts. S9 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_parent_lift.py`: combining the S7 branches with the S8 contours gives `0/48` parent-lift budget passes; the best branch still misses the square-contour cover condition by `27.38x` and the local residual budget by `43.48x`. S10 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_tail_conditioning.py`: raw Coulomb-tail forcing is small (`max tail/budget = 1.64e-2`), but Riccati transfer is not uniformly stable (`4/48` passes; `|m_{400}-m_{800}|/budget = 0.9896..88.60`). S11 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_transfer_condition.py`: the finite dyadic transfer improves from `4/48` passes on `400->800` to `24/48` on `800->1600`, but the infinitesimal condition-number route is too pessimistic (`K` up to `6.45e6`, linear pass `0/48` on `800->1600`). S12 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_projective_transfer.py`: the exact finite-amplitude Jost identity gives `48/48` endpoint passes on `1600->3200` with max ratio `1.435e-3`, median `6e-6`, and max formula absolute error `1.78e-8` in local-budget units; an independent Riccati rerun at `1600->3200` also gives `48/48` passes, while the all-segment denominator floor remains too conservative (`24/48`). S13 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_jost_product_bound.py`: the endpoint Jost-product lower bound passes `48/48`, with required product `3.10..103.58`, observed product `5.5867e3..2.5823e15`, minimum product margin `697.076`, and minimum projective pole-exclusion margin `697.074`. S14 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_contour_jost_bound.py`: the contour-uniform Jost-product lift passes `48/48` local boxes at `1600->3200`, with max boundary ratio `2.6058e-2`, minimum contour product margin `38.3758`, zero Jost winding on every box, and a `64`-point bottleneck rerun giving the same displayed worst values. S15 adds `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_r_coherence_single_pole.py`: the single-pole `R`-coherence necessary-condition audit has S14 tail-box control on all `39/39` coherence groups but `0/39` common box intersections; full-case exact square-cover factors are `36.5644`, `36.9301`, and `37.2734` for `D=6,12,18`, and the pure fixed-`R_C`, fixed-parity `R`-sweep still needs factor `28.7993` at best. S16--S25 then formalize the parent-side Rouché/implicit-flow gate and add `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_r_flow_bound.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_large_tube_rouche.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_adapted_flow.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_curved_flowbox.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_slab_envelope.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_cauchy_slab_bound.py`, `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_interval_promotion.py`, and `/Users/boypatrick/codex/PSLT_quantam/code/audit_static_width_whittaker_special_interval_certificate.py`: all `12/12` fixed-`(D,R_C,parity)` paths retain S14 endpoint tail-box control, but `0/12` have adjacent local-box overlap and the required finite-difference R-flow tube is `59.5--75.1` current box half-widths; the large-tube audit gets close on the sampled grid (`33/36` adaptive coarse segment closures) but the refined `D=6,R_C=400` rectangle bottleneck remains open (`2/6` closures, max best index-compatible Rouché ratio `1.475069`); the adapted-secant implicit-flow audit then closes only the late `100->120` even/odd pieces (`2/6`), leaving the early `60->80` and `80->100` flowbox as the parent-side bottleneck (max best contraction `2.408830`, median `1.155677`); S20 integrates the curved-flowbox envelope and closes both `80->100` parity pieces at `c=96`; S21 reruns the final `60->80` pair on parent-scale tubes and closes both parities (`2/2`, max best ratio `0.918494`, median `0.905757`, min derivative floor `97.09324`); S22 rewrites that closure as a `32`-slab, `5%`-inflated upper-Darboux envelope at `c=640`, again closing `2/2` parities with max inflated ratio `0.969175`, median `0.955463`, max uninflated upper-Darboux ratio `0.923024`, min derivative floor `97.09324`, and max CR mismatch `2.8890e-2`; S23 converts the `5%` reserve into a Cauchy/slab derivative-remainder bound with fivefold node-slope guard, max reserve usage `0.576824`, max beta-to-S22 ratio `0.979849`, and max integrated Cauchy ratio `0.946761`; S24 intervalizes that derivative envelope into one parity-uniform parent constant per parity (`2.64063e-4`, `3.20405e-4`), closing `2/2` parity branches and all `64/64` slab inequalities with max reserve usage `0.793969`, max beta-to-S22 ratio `0.990189`, and max interval integrated ratio `0.954314`; S25 checks those constants directly against the Whittaker-DtN special-function backend with `480` direct derivative samples per parity and a factor-two guard, closing `2/2` parity branches with max guarded-direct/S24 ratio `0.888891`, max direct reserve usage `0.542851`, and max direct integrated ratio `0.938387`. The static-width line should stop new analysis at this point: remaining work is prose polish or future domain broadening only, not further CAP, truncated-tail, backend, or unconstrained branch-picking support for the present S25 claim.
- **g_N migration comparator:** `code/scan_gn_profile_impact.py` compares `g_mode=fp_2d_full/cardy/fp_1d/fp_2d`; relative to baseline `fp_2d_full`, the tested comparator profiles leave $f(\mathcal{R}_3>0.90)$ unchanged at displayed precision, move $f(\mathcal{R}_3>0.95)$ only at the percent level, and keep high-$N$ runaway controlled at `f(N_win>3)\approx2.78\times10^{-4}`.
...
- **g_N exact low-N finite-volume count:** `code/audit_gn_lowN_microstate_count.py` certifies the first three two-dimensional low modes as exact projector ranks `1,2,3` on the fine `D` grid (`min gap12 = 4.16e-2`, `min gap23 = 4.97e-2`) while keeping the release `fp_2d_full` shell-volume baseline distinct from a pure integer-count replacement.
...
- **EYMH direct-scalar topology-adoption certificate:** the U10 direct inserted-history `y_2^{raw}(D_*)` scalar remains a fixed-bridge topology change, not a silent replacement. The U12 projector-continuation audit under
  - `code/audit_hll_y2raw_projector_continuation.py`
  - `output/hll_absolute_normalization/hll_y2raw_projector_continuation_summary.csv`
  shows the first no-topology crossing at `s=0.457084`, before the direct endpoint `s=1`. The U13 policy audit under
  - `code/audit_hll_y2raw_topology_adoption_policy.py`
  - `output/hll_absolute_normalization/hll_y2raw_topology_adoption_policy_summary.csv`
  certifies explicit topology-changing adoption: the accepted set expands from `{D=8.8,9.6} x E21` to `{D=8.0,8.8,9.6} x E21`, with `21` gains, `0` losses, one accepted component before and after, and max crossing validation error `4.44e-16`. The U14 margin audit under
  - `code/audit_hll_y2raw_topology_margin_certificate.py`
  - `output/hll_absolute_normalization/hll_y2raw_topology_margin_certificate_summary.csv`
  then shows this is not a boundary-grazing adoption: the direct branch has uniform `mu` set-invariance radius `0.145404`, larger than the release radius `0.104937`, and the added slab sits `0.363509 sigma` inside the `2sigma` band. This makes the direct scalar reviewer-qualified only when the topology-changing slab expansion is reported explicitly; conservative no-topology wording should keep the release scalar.
