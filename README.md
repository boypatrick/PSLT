...
- **Primary full_direct release mode:** `code/publish_full_direct_map.py` publishes one integrated release table combining:
  ...
  Both scan scripts now expose a release-candidate per-cell kinetics mode via `chain_mode=cell_direct_runtime`: `chi_LR(D)` and `A_l(D)` are evaluated by direct solvers inside scan evaluation, while `g_N/B_N` stay on release profile closures for map-level parity with `full_direct`. The stress-only all-direct path is kept as `chain_mode=cell_direct_runtime_extreme`, where `g_N(D)` (via `g_mode=fp_2d_full_runtime_direct`) and EFT-operator visibility inputs `(g_{iN}(D), \lambda_N(D))` (via `b_mode=eft_operator_norm_runtime_direct`) are also solved per cell; with `--runtime-direct-no-cache`, this becomes strict per-call recomputation (very expensive; diagnostic only).
