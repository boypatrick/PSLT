...
| V4: two-center ramification-flux lock | `FUTURE FIRST-PRINCIPLES CANDIDATE` | Try to derive `R=p_+ + p_-` and `L_R=O(2)` from the two-center parent data.  Target theorem: each center contributes one unit of projective ramification/flux; on `CP^1`, `Pic(CP^1)=Z`, so the degree-two divisor determines `L_R=O(R)=O(2)` uniquely. | Mathematical note plus an executable algebraic-topology sanity gate for `deg R=2`, `Pic(CP^1)=Z`, `h^0(O(2))=3`, `h^1(O(2))=0`.  Must distinguish "two centers imply degree two" from merely choosing a degree-two line bundle. |
...
| V8: submission stop rule | `ACTIVE GUARD` | If the goal is final submission packaging, stop at V0--V3 and keep V4--V7 optional.  Do not change main claims, figures, or scan normalizations before submission. | No manuscript change. |
