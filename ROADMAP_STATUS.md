...
| V5: `Spin^c` carrier uniqueness vs ordinary spin | `FUTURE FIRST-PRINCIPLES CANDIDATE` | Try to derive why the protected family carrier is the Kähler/Dolbeault `Spin^c` carrier `Lambda^{0,*}T^*C \otimes L_R`, not the ordinary spin carrier `K^{1/2} \otimes L_R`.  Target theorem: projective/Kähler quantization and SM chirality require the Dolbeault `Spin^c` carrier, whose Todd shift gives `deg L_R + 1 = 3`; the ordinary spin choice gives `h^0(O(1))=2` and is incompatible with the V1/V2 three-mode spectral embedding. | Mathematical note plus gate comparing the two kernels: `Spin^c` gives `(h^0,h^1)=(3,0)` while ordinary spin on `CP^1` with `O(2)` gives two holomorphic modes.  Must not phrase the exclusion as "ordinary spin is mathematically illegal"; it is legal but belongs to the wrong carrier. |
...
| V8: submission stop rule | `ACTIVE GUARD` | If the goal is final submission packaging, stop at V0--V4 and keep V5--V7 optional.  Do not change main claims, figures, or scan normalizations before submission. | No manuscript change. |
