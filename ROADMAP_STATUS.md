...
| Protected `Spin^c` index family-count upgrade | `OPTIONAL / FINITE-VOLUME POSITIVE BRIDGE PACKAGE` | A conditional first-principles theorem may be developed later under explicit inputs `C=CP^1`, `R=p_+ + p_-`, `L_R=O(2)`, a protected `Spin^c` Dolbeault-Dirac carrier, and no extra unpaired SM-charged kernels.  The standalone bridge package is now finite-volume positive: V0 closes dimension/flag compatibility, V1 supplies a fixed tapered-polynomial anchor-overlap certificate, and V2 confirms the physical `even,odd,even` / `0,1,2` nodal-Sturm ordering against the audited `N=1,2,3` spectral subspace.  This remains optional and does not alter the submission baseline or A1--A5 wording. |
...
| Protected `Spin^c` index three-family theorem | Would replace the weakest scan-level family-count intuition by a conditional index theorem: `R=p_+ + p_-`, `L_R=O(2)`, `h^0(CP^1,O(2))=3`, `h^1(CP^1,O(2))=0`, and a product Dirac reduction to three unpaired SM-charged chiral families.  It remains optional because these are explicit geometric/protection assumptions, not consequences of the current EYMH parent.  The bridge package is now finite-volume positive: V0 closes dimension/flag compatibility, V1 supplies a fixed tapered-polynomial anchor-overlap certificate, V2 confirms the physical `even,odd,even` / `0,1,2` nodal-Sturm ordering, V3 conditionally selects `C=CP^1`, V4 conditionally locks `R=p_+ + p_-`, `L_R=O(2)`, and V5 conditionally selects the Dolbeault `Spin^c` carrier over ordinary spin with the same V4 flux.  Remaining first-principles completion item is V6: no-exotics Bochner/Lichnerowicz gap. | Reopen only if a revision asks for a stronger exactly-three mechanism, or if we deliberately write a companion mechanism paper.  Current gates: `python3 code/check_spinc_bridge_lemma.py` returns `CONDITIONAL_DIMENSION_FLAG_BRIDGE_ONLY`; `python3 code/check_spinc_anchor_overlap.py` returns `PROMOTABLE_ANCHOR_INTERTWINER`; `python3 code/check_spinc_nodal_sturm_order.py` returns `NODAL_STURM_ORDER_CONFIRMED`; `python3 code/check_spinc_projective_minimality.py` returns `CONDITIONAL_CP1_SELECTED_BY_NO_HIDDEN_MODULI`; `python3 code/check_spinc_ramification_flux_lock.py` returns `CONDITIONAL_O2_LOCKED_BY_UNIT_TWO_CENTER_FLUX`; `python3 code/check_spinc_carrier_uniqueness.py` returns `CONDITIONAL_SPINC_CARRIER_SELECTED_OVER_ORDINARY_SPIN`.  V6 must remain outside the submission baseline until no-exotics is converted into an audited parent-side theorem.  No scan retuning and no submission-baseline change. |
...
| V6: no-exotics Bochner/Lichnerowicz gap | `FUTURE FIRST-PRINCIPLES CANDIDATE` | Try to prove that all non-protected SM-charged internal sectors have zero net index or a positive vectorlike mass gap.  Target theorem: after Birkhoff-Grothendieck splitting on `CP^1`, every extra SM-charged bundle either pairs as `F \oplus K \otimes F^*` with zero chiral index or satisfies a Lichnerowicz/Bochner lower bound `D_F^2 >= delta > 0`. | Mathematical note plus a certificate table listing allowed non-protected bundle splittings, their indices, pairings, and gap lower-bound assumptions.  This is the only route that can honestly support "no unpaired SM-charged exotics" beyond an explicit assumption. |
| V7: negative diagnostic closure | `FALLBACK` | If V3--V6 cannot derive the missing inputs, close the route as a companion protected-family mechanism rather than a promotion of the present spectral-layer baseline. | Roadmap wording update only: `finite-volume bridge positive; first-principles uniqueness not derived`. |
| V8: submission stop rule | `ACTIVE GUARD` | If the goal is final submission packaging, stop at V0--V5 and keep V6--V7 optional.  Do not change main claims, figures, or scan normalizations before submission. | No manuscript change. |
