...
| V1: moment / polynomial anchor overlap | `CLOSED NEGATIVE` | Construct a concrete pullback/embedding `T_D` of the polynomial basis `{1,zeta,zeta^2}` into the finite-volume solver grid and test the overlap matrix `A_{nk}(D)=<u_n(D),T_D e_k>`.  The fixed tapered-polynomial degree-flag anchor is not promotable: on the audited G1 `D` grid, `min_D sigma_min(A(D))=2.6054145061956392e-15`, `min_D |det A(D)|=3.254357574712386e-18`, `max_D ||P_{>3}T_D||=0.9999993069690623`, and rank drops occur at `D=4.8,9.6,14.4,19.2`. | Script `code/check_spinc_anchor_overlap.py`; note `paper/protected_spinc_anchor_overlap_note.md`; artifacts `output/spinc_bridge/spinc_anchor_overlap_summary.csv` and detail table.  Interpretation: dimension/flag compatibility remains valid, but this canonical moment-polynomial lift does not give a canonical spectral embedding. |
| V2: nodal/Sturm ordering anchor | `NEXT / SECONDARY` | Since V1 closed negative, test whether the protected basis can still be paired to the spectral chain by nodal count, parity/sign convention, or Sturm ordering: `1 <-> u_1`, `zeta <-> u_2`, `zeta^2 <-> u_3`.  This is more physically readable but weaker unless phase and branch labels are made invariant. | Future diagnostic note plus nodal/order certificate; only use as support for a companion mechanism or as a negative diagnostic. |
