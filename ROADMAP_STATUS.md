...
| Assumption-frontload pass | `ACTIVE / WORDING GATE` | Add a compact assumption map near the front of the manuscript.  The submission-safe reading is conditional: A1 fixed projected two-source conformal background; A2 spectral-layer labels are EFT generation labels rather than spin transmutation; A3 `g_N` is a specified shell-volume/phase-space weight with tail prescription; A4 Rank-2 matrix is the coarse-grained kinetic closure; A5 the visibility sector is an EFT/Wilson-inspired scan kernel.  Theorems/certificates should be read as A1--A5 plus audited-domain statements, not as unconditional EYMH action-level derivations. |
| Static-width wording consistency | `ACTIVE / WORDING GATE` | Grep and harmonize the S1--S25 language so that older `not_promoted` statements are clearly audit-history conclusions superseded only on the certified S25 parent tube.  The final wording must keep S25 local: `D=6, R_C=400, R=60->80, c=640`; outside that tube, `Gamma_{N,l}^{(geo)}` remains the fallback/comparator.  No global pole family, backend replacement, or branch-picking theorem is reopened. |
| `H->mumu` statistical wording guard | `ACTIVE / WORDING GATE` | The ATLAS `mu_mumu=1.4+-0.4` comparison must be described as a reference-normalized illustrative constraint / scan-level diagnostic, not as experimental support for PSLT.  Keep Fig. 17/reference-normalization sensitivity visible, state the fixed-reference convention for headline maps, and avoid any phrase implying a formal likelihood, CL exclusion, or blind absolute prediction unless such a likelihood is explicitly introduced. |
| Submission positioning sentence | `ACTIVE / WORDING GATE` | Ensure the abstract, introduction, conclusion, limitations, and front status table all contain the same positioning sentence: the paper is a computable EFT-level spectral-layer occupancy closure with first-three-layer concentration and an audited finite-domain no-fourth-bound-layer certificate, not a full action-level theorem proving exactly three Standard Model generations. |
...
8. Add the assumption-frontload paragraph/table so A1--A5 are explicit before the main claims.
9. Run the static-width wording consistency pass: S25 local promotion only, `Gamma_{N,l}^{(geo)}` fallback outside the certified tube.
10. Run the `H->mumu` statistical wording guard: fixed-reference illustrative constraint only, no support/exclusion language.
11. Re-check the submission positioning sentence across abstract, introduction, conclusion, limitations, and front status table.
