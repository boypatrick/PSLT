...
def _negative_overlap_grid(
    ...
    negative_overlap_density = np.clip(-(sign_diag * C / (2.0 * np.pi * np.maximum(rr, 1e-300))), 0.0, None)
    ...
    for rho, zeta, val in zip(rr[band].ravel(), local_zeta[band].ravel(), negative_overlap_density[band].ravel()):
...
def main() -> None:
    ...
    detail = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_band_negative_carrier_source_detail.csv")
    rho_local, zeta_local, overlap_grid = _negative_overlap_grid(
        D=float(args.repr_D),
    ...
    ax = axes[0]
    ax.plot(detail["D"], detail["rho_bar_weighted"], marker="o")
    ax.set_xlabel(r"$D$")
    ax.set_ylabel(r"$\bar\rho_{\rm band}^{(-)}$")
    ax.set_title(r"Weighted radial factor in $W_{\rm band}^{(-)}=2\pi\bar\rho M_{\rm band}^{(-)}$")

    ax = axes[1]
    ax.plot(detail["D"], detail["W_band_neg"], marker="o", label=r"$W_{\rm band}^{(-)}$")
    ax.plot(detail["D"], detail["M_band_neg"], marker="s", label=r"$M_{\rm band}^{(-)}$")
    ax.plot(detail["D"], 2*np.pi*detail["rho_bar_weighted"]*detail["M_band_neg"], marker="^", label=r"$2\pi\bar\rho M_{\rm band}^{(-)}$")
    ax.set_xlabel(r"$D$")
    ax.set_ylabel("magnitude")
    ax.set_title(r"Negative carrier moment vs radial factorization")
    ax.legend(fontsize=8, loc="best")

    ax = axes[2]
    positive_vals = overlap_grid[overlap_grid > 0.0]
    vmin = float(max(positive_vals.min(), 1e-12)) if positive_vals.size else 1e-12
    vmax = float(max(overlap_grid.max(), vmin * 10.0))
    im = ax.pcolormesh(
        rho_local,
        zeta_local,
        np.where(overlap_grid > 0.0, overlap_grid, np.nan),
        shading="nearest",
        cmap="magma",
    ...
    ax.set_title(rf"Local negative overlap density at $D={args.repr_D:.0f}$")
    cbar = fig.colorbar(im, ax=ax, pad=0.02)
    cbar.set_label(r"$M_{\rm band}^{(-)}$ density")

    out = OUTDIR / "chi_open_system_parity_contrast_diag_anchor_band_negative_carrier_source_figure.png"
