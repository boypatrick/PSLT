...
def _local_negative_density(
    ...
    for rho, zeta, val in zip(rr[band].ravel(), local_zeta[band].ravel(), q_mass_neg[band].ravel()):
...
def main() -> None:
    ...
    detail = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_band_corner_source_detail.csv")
    scan = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_band_corner_source_scan.csv")

    pairs = sorted(scan[["rho_corner", "zeta_corner"]].drop_duplicates().itertuples(index=False, name=None))
    rho_vals = sorted({float(r) for r, _ in pairs})
    zeta_vals = sorted({float(z) for _, z in pairs})
    floor_grid = np.full((len(zeta_vals), len(rho_vals)), np.nan)
    cap_grid = np.full((len(zeta_vals), len(rho_vals)), np.nan)
    for _, row in scan.iterrows():
        i = zeta_vals.index(float(row["zeta_corner"]))
        j = rho_vals.index(float(row["rho_corner"]))
        floor_grid[i, j] = float(row["N_mass_corner_floor_abs"])
        cap_grid[i, j] = float(row["max_cap_over_P_mass_inner"])
    ...
    ax = axes[0]
    im0 = ax.pcolormesh(rho_vals, zeta_vals, floor_grid, shading="nearest", cmap="viridis")
    ax.axvline(float(args.canonical_rho_corner), color="white", linestyle="--", linewidth=1.2)
    ax.axhline(float(args.canonical_zeta_corner), color="white", linestyle="--", linewidth=1.2)
    ax.set_xlabel(r"$\rho_{\rm corner}$")
    ax.set_ylabel(r"$\zeta_{\rm corner}$")
    ax.set_title(r"Strict corner-subband floor $\min_D N_{\rm corner}$")
    cbar0 = fig.colorbar(im0, ax=ax, pad=0.02)
    cbar0.set_label(r"$\min_D N_{\rm corner}$")

    ax = axes[1]
    ax.plot(detail["D"], detail["N_mass_band"] / detail["P_mass_inner"], marker="o", label=r"$N_{\rm band}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_corner_over_P_mass_inner"], marker="s", label=r"$N_{\rm corner}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_cap_over_P_mass_inner"], marker="^", label=r"$N_{\rm cap}/P_{\rm mass,in}$")
    ax.axvline(11.0, color="0.5", linestyle="--", linewidth=1.0)
    ax.set_xlabel(r"$D$")
    ax.set_ylabel("ratio")
    ax.set_title(r"Canonical strict corner $(0.22;0.10)$ inside the band")
    ...
    ax.axvline(float(args.canonical_rho_corner), color="white", linestyle="--", linewidth=1.1)
    ax.axhline(float(args.zeta_band_low), color="cyan", linestyle="--", linewidth=1.1)
    ax.axhline(float(args.canonical_zeta_corner), color="white", linestyle="--", linewidth=1.1)
    ...
    cbar2.set_label(r"$[q_{\rm mass}]_-$")

    out = OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_band_corner_source_figure.png"
