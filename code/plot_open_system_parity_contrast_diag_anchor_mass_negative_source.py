...
def main() -> None:
    ...
    ap.add_argument("--rho-min", type=float, default=0.15)
    ap.add_argument("--zeta-box", type=float, default=1.20)
    ...
    detail = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_negative_source_detail.csv")
    scan = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_negative_source_scan.csv")

    rho_vals = np.sort(scan["rho_min"].unique())
    zeta_vals = np.sort(scan["zeta_box"].unique())
    heat = (
        scan.pivot(index="rho_min", columns="zeta_box", values="max_tail_over_P_mass_inner")
    ...
    # Panel A: scan heatmap for annular tail leakage.
    ax = axes[0]
    X, Y = np.meshgrid(zeta_vals, rho_vals)
    pcm = ax.pcolormesh(X, Y, heat, shading="nearest", cmap="viridis")
    ax.scatter([float(args.zeta_box)], [float(args.rho_min)], color="white", edgecolor="black", s=55, zorder=3)
    ax.axhline(0.12, color="white", linestyle="--", linewidth=1.0, alpha=0.8)
    ax.set_xlabel(r"$\zeta_{\rm box}$")
    ax.set_ylabel(r"$\rho_{\min}$")
    ax.set_title(r"Max $N_{\rm tail}/P_{\rm mass,in}$ scan")
    cbar = fig.colorbar(pcm, ax=ax, pad=0.02)
    cbar.set_label(r"$\max_D\, N_{\rm tail}/P_{\rm mass,in}$")

    # Panel B: D-series decomposition.
    ax = axes[1]
    ax.plot(detail["D"], detail["N_mass_inner"] / detail["P_mass_inner"], marker="o", label=r"$N_{\rm mass,in}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_annulus_over_P_mass_inner"], marker="s", label=r"$N_{\rm mass,ann}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_tail_over_P_mass_inner"], marker="^", label=r"$N_{\rm mass,tail}/P_{\rm mass,in}$")
    ax.axvline(11.0, color="0.5", linestyle="--", linewidth=1.0)
    ax.set_xlabel(r"$D$")
    ax.set_ylabel("ratio")
    ax.set_title(r"Canonical adverse-annulus split $(0.15,1.20)$")
    ax.legend(fontsize=8, loc="best")

    # Panel C: representative local adverse density map.
    ax = axes[2]
    ...
    ax.axvline(float(args.rho_min), color="cyan", linestyle="--", linewidth=1.2)
    ax.axvline(float(args.rho_anchor), color="white", linestyle=":", linewidth=1.0)
    ax.axhline(float(args.zeta_box), color="cyan", linestyle="--", linewidth=1.2)
    ...
    out = OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_negative_source_figure.png"
