...
def main() -> None:
    ...
    detail = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_shoulder_source_detail.csv")
    scan = pd.read_csv(OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_shoulder_source_scan.csv")

    rho_vals = np.sort(scan["rho_shoulder"].unique())
    rim_vals = [float(scan.loc[np.isclose(scan["rho_shoulder"], r), "max_rim_over_P_mass_inner"].iloc[0]) for r in rho_vals]
    share_vals = [float(scan.loc[np.isclose(scan["rho_shoulder"], r), "min_shoulder_over_ann_share"].iloc[0]) for r in rho_vals]
    ...
    ax = axes[0]
    ax.plot(rho_vals, rim_vals, marker="o", label=r"$\max_D\,N_{\rm rim}/P_{\rm mass,in}$")
    ax.plot(rho_vals, share_vals, marker="s", label=r"$\min_D\,N_{\rm shoulder}/N_{\rm ann}$")
    ax.axvline(float(args.canonical_rho_shoulder), color="0.4", linestyle="--", linewidth=1.0)
    ax.set_xlabel(r"$\rho_{\rm shoulder}$")
    ax.set_ylabel("ratio")
    ax.set_title(r"Radial shoulder scan inside $A_{\rm neg}^{(0.15,1.20)}$")
    ax.legend(fontsize=8, loc="best")

    ax = axes[1]
    ax.plot(detail["D"], detail["N_mass_ann"] / detail["P_mass_inner"], marker="o", label=r"$N_{\rm ann}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_shoulder_over_P_mass_inner"], marker="s", label=r"$N_{\rm shoulder}/P_{\rm mass,in}$")
    ax.plot(detail["D"], detail["canonical_rim_over_P_mass_inner"], marker="^", label=r"$N_{\rm rim}/P_{\rm mass,in}$")
    ax.axvline(11.0, color="0.5", linestyle="--", linewidth=1.0)
    ax.set_xlabel(r"$D$")
    ax.set_ylabel("ratio")
    ax.set_title(r"Canonical adverse shoulder $(0.20,1.20)$")
    ax.legend(fontsize=8, loc="best")
    ...
    ax.axvline(float(args.annulus_rho_min), color="white", linestyle=":", linewidth=1.0)
    ax.axvline(float(args.canonical_rho_shoulder), color="cyan", linestyle="--", linewidth=1.2)
    ...
    out = OUTDIR / "chi_open_system_parity_contrast_diag_anchor_mass_shoulder_source_figure.png"
