...
def scan_D(
    ...
    """
    Scan over D values and compute eigenvalues, WKB action.
    
    If allow_unstable_S=True, compute S even for unstable modes (omega^2 < 0).
    ...
    for D in D_list:
        ...
        for n, E in enumerate(Evals, start=1):
            ...
            # Compute S based on mode type
            if has_stable_bound:
                S, tp = wkb_action_central(U, z, float(E))
            elif allow_unstable_S and has_bound:
                # Diagnostic mode: compute S even for unstable bound states
                S, tp = wkb_action_central(U, z, float(E))
            else:
                S, tp = (np.nan, None)
            ...
            is_diag_tunnel = bool(has_bound and has_tp and np.isfinite(S) and (S > 0.0))
...
def main():
    ...
    ap.add_argument("--allow_unstable_S", action="store_true",
                    help="Compute S even for unstable modes (omega^2 < 0). For diagnostic only.")
    ...
    # For diagnostic_deepwell preset, enable unstable S by default
    allow_unstable = args.allow_unstable_S or (args.preset == "diagnostic_deepwell")
    df, rep_data = scan_D(D_list, a, eps, m0, xi, zmax, Nz, nstates, reps, allow_unstable_S=allow_unstable)
