#!/usr/bin/env python3
"""
Microscopic open-system chi sensitivity scan.

This script compares baseline localized chi(D) against open-system chi(D)
constructed from microscopic profile-interpolated
(delta_micro, gamma_phi_micro, gamma_mix_micro, gamma_ref_micro)
through the Lindblad proxy in pslt_lib (chi_mode="open_system_micro").

Outputs:
  - output/chi_open_system/chi_open_system_micro_sensitivity.csv
  - output/chi_open_system/chi_open_micro_ratio_profiles.csv
  - output/chi_open_system/chi_open_micro_ratio_band.csv
  - paper/chi_open_system_micro_sensitivity.csv
"""
...
class Case:
    name: str
    mode: str  # localized | open_system_micro
    phi_scale: float = 1.0
    mix_scale: float = 1.0


def pick_open_micro_csv() -> Path:
...
def main() -> None:
    ...
    open_csv = pick_open_micro_csv()

    cases = [
        Case("baseline_localized", mode="localized"),
        Case("open_micro_base", mode="open_system_micro", phi_scale=1.0, mix_scale=1.0),
        Case("open_micro_phi_x0p5", mode="open_system_micro", phi_scale=0.5, mix_scale=1.0),
        Case("open_micro_phi_x2", mode="open_system_micro", phi_scale=2.0, mix_scale=1.0),
        Case("open_micro_mix_x0p5", mode="open_system_micro", phi_scale=1.0, mix_scale=0.5),
        Case("open_micro_mix_x2", mode="open_system_micro", phi_scale=1.0, mix_scale=2.0),
    ]
    ...
    out_sens = OUTDIR / "chi_open_system_micro_sensitivity.csv"
    with out_sens.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=list(metric_rows[0].keys()))
        writer.writeheader()
        writer.writerows(metric_rows)

    out_ratio = OUTDIR / "chi_open_micro_ratio_profiles.csv"
    ...
    base_ratio = [r for r in ratio_rows_all if r["case"] == "open_micro_base"]
    arr = np.asarray([r["ratio_to_baseline"] for r in base_ratio], dtype=float)
    out_band = OUTDIR / "chi_open_micro_ratio_band.csv"
    pd.DataFrame(
        [
            {
                "case": "open_micro_base",
    ...
    paper_csv = PAPER_DIR / "chi_open_system_micro_sensitivity.csv"
