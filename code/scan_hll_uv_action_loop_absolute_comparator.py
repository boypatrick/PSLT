#!/usr/bin/env python3
"""
Compare UV matching modes including action_loop_absolute on a common D21xE21 grid.

This is comparator-only. It checks whether the contrast-modulated absolute
loop-prefactor witness remains a small deformation relative to the existing
action_absolute / action_loop_contrast hierarchy.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Compare constant/action_absolute/action_loop_contrast/action_loop_absolute UV matching modes")
...
def plot_pairwise(out_png: Path, merged: pd.DataFrame) -> None:
    ...
    panels = [
        ...
        ("abs vs loop_contrast", arr_from_series((merged["mu_mumu_uv_rge_action_absolute"] - merged["mu_mumu_uv_rge_action_loop_contrast"]).abs())),
        ("contrast vs loop_absolute", arr_from_series((merged["mu_mumu_uv_rge_action_loop_contrast"] - merged["mu_mumu_uv_rge_action_loop_absolute"]).abs())),
    ...
    fig.suptitle("UV matching comparator including action_loop_absolute", fontsize=13)
...
def main() -> None:
    ...
    merged = const.merge(absv, on=["D", "eta"]).merge(contrast, on=["D", "eta"]).merge(loop_abs, on=["D", "eta"])
    ...
    out_mode = OUTDIR / f"hll_uv_action_loop_absolute_mode_summary_{args.tag}.csv"
    out_pair = OUTDIR / f"hll_uv_action_loop_absolute_pairwise_summary_{args.tag}.csv"
    out_fig = OUTDIR / f"hll_uv_action_loop_absolute_comparator_{args.tag}.png"
    out_meta = OUTDIR / f"hll_uv_action_loop_absolute_comparator_run_meta_{args.tag}.json"
