#!/usr/bin/env python3
"""
Compare UV matching modes including action_loop_contrast on a common D21xE21 grid.

This is a comparator-only script. It does not change the baseline; it audits
whether the contrast-based heat-kernel witness can serve as a plausible next
normalization source beyond action_absolute.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Compare constant/action_absolute/action_loop_contrast UV matching modes")
...
def summarize_mode(df: pd.DataFrame, label: str, basis_summary_path: Path) -> dict[str, float | str]:
    ...
    if basis_summary_path.exists():
        ...
        for c in [
            ...
            if c in bs.index:
                row[c] = float(bs[c])
    for c in ["kappa_diag_eff", "action_abs_diag", "hk_curv_access", "hk_curv_contrast_log"]:
        col = f"{c}_{label}"
...
def plot_pairwise(out_png: Path, merged: pd.DataFrame) -> None:
    ...
    panels = [
        ...
        ("const vs loop_contrast", arr_from_series((merged["mu_mumu_uv_rge_constant"] - merged["mu_mumu_uv_rge_action_loop_contrast"]).abs())),
        ("action_abs vs loop_contrast", arr_from_series((merged["mu_mumu_uv_rge_action_absolute"] - merged["mu_mumu_uv_rge_action_loop_contrast"]).abs())),
        ("hk_curv_access", arr_from_series(merged["hk_curv_access_action_loop_contrast"])),
    ]
    ...
    fig.suptitle("UV matching comparator including action_loop_contrast", fontsize=13)
...
def main() -> None:
    ...
    loop = load_map(args.contrast_map, "action_loop_contrast")
    merged = const.merge(absv, on=["D", "eta"]).merge(loop, on=["D", "eta"])
    ...
    out_mode = OUTDIR / f"hll_uv_action_loop_contrast_mode_summary_{args.tag}.csv"
    out_pair = OUTDIR / f"hll_uv_action_loop_contrast_pairwise_summary_{args.tag}.csv"
    out_fig = OUTDIR / f"hll_uv_action_loop_contrast_comparator_{args.tag}.png"
    out_meta = OUTDIR / f"hll_uv_action_loop_contrast_comparator_run_meta_{args.tag}.json"
