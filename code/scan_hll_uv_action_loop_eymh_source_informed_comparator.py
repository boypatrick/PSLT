#!/usr/bin/env python3
"""
Compare UV matching modes including action_loop_eymh_source_informed on a common D21xE21 grid.

This is comparator-only. It checks whether the source-informed EYMH-side
absolute loop-prefactor witness remains a small deformation relative to the
current action_loop_eymh_absolute comparator.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Compare constant/action_loop_eymh_absolute/action_loop_eymh_source_informed UV matching modes"
    )
    ...
    ap.add_argument("--source-informed-map", type=Path, default=OUTDIR / "hll_uv_to_eft_map_uv_action_loop_eymh_source_informed_D21E21.csv")
    ...
    ap.add_argument("--source-informed-basis-summary", type=Path, default=OUTDIR / "hll_uv_operator_basis_summary_uv_action_loop_eymh_source_informed_D21E21.csv")
...
def plot_pairwise(out_png: Path, merged: pd.DataFrame) -> None:
    ...
    panels = [
        ...
        ("eymh vs source-informed", arr_from_series((merged["mu_mumu_uv_rge_action_loop_eymh_absolute"] - merged["mu_mumu_uv_rge_action_loop_eymh_source_informed"]).abs())),
        ...
        ("tree_diag_compressibility", arr_from_series(merged["tree_diag_compressibility_action_loop_eymh_source_informed"])),
    ]
    ...
    fig.suptitle("UV matching comparator including action_loop_eymh_source_informed", fontsize=13)
...
def main() -> None:
    ...
    merged = const.merge(eymh, on=["D", "eta"]).merge(src, on=["D", "eta"])
    ...
    out_mode = OUTDIR / f"hll_uv_action_loop_eymh_source_informed_mode_summary_{args.tag}.csv"
    out_pair = OUTDIR / f"hll_uv_action_loop_eymh_source_informed_pairwise_summary_{args.tag}.csv"
    out_fig = OUTDIR / f"hll_uv_action_loop_eymh_source_informed_comparator_{args.tag}.png"
    out_meta = OUTDIR / f"hll_uv_action_loop_eymh_source_informed_comparator_run_meta_{args.tag}.json"
