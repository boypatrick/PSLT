...
def _point_payload(kin, layer: int, D: float, eta: float) -> dict[str, float]:
    ...
    weight = float(kin.layer_weight(layer, D, eta, T_COH))
    amp = float(
                    ...
                    rows.append(
                        {
                            ...
                            "mu": cur["mu"],
                            "chi2": cur["chi2"],
                            "log_g_eff_ratio": _safe_log_ratio(cur["g_eff"], base["g_eff"]),
                            "log_gamma_ratio": _safe_log_ratio(cur["gamma"], base["gamma"]),
                            "log_p_kin_ratio": _safe_log_ratio(cur["p_kin"], base["p_kin"]),
                            "log_B_ratio": _safe_log_ratio(cur["B"], base["B"]),
                            "log_W_ratio": _safe_log_ratio(cur["W"], base["W"]),
                            "log_amp_ratio": _safe_log_ratio(cur["amp"], base["amp"]),
...
def _summary_rows(detail: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, float | int | str]] = []
    components = ("g_eff", "gamma", "p_kin", "B", "W", "amp")
    for layer in LAYERS:
        for mode_key in COMPARE_MODES:
            sub = detail[(detail["layer"] == layer) & (detail["mode"] == mode_key)].copy()
            row: dict[str, float | int | str] = {"layer": layer, "mode": mode_key}
            for comp in components:
                row[f"p95_abs_log_{comp}_ratio"] = float(np.percentile(np.abs(sub[f"log_{comp}_ratio"]), 95.0))
                row[f"max_abs_log_{comp}_ratio"] = float(np.max(np.abs(sub[f"log_{comp}_ratio"])))
            row["p95_abs_delta_mu"] = float(np.percentile(np.abs(sub["delta_mu"]), 95.0))
            row["max_abs_delta_mu"] = float(np.max(np.abs(sub["delta_mu"])))
            row["p95_abs_delta_log_mu"] = float(np.percentile(np.abs(sub["delta_log_mu"]), 95.0))
            row["acceptance_mismatch"] = float(np.mean(sub["acceptance_mismatch"])) if layer == 2 else 0.0
            rows.append(row)
    return pd.DataFrame(rows)
        ...
        rows.append(
            {
                "mode": mode_key,
                "D": float(d_val),
                "slice_p95_abs_log_B_ratio": float(np.percentile(np.abs(grp["log_B_ratio"]), 95.0)),
                "slice_p95_abs_log_W_ratio": float(np.percentile(np.abs(grp["log_W_ratio"]), 95.0)),
                "slice_p95_abs_log_amp_ratio": float(np.percentile(np.abs(grp["log_amp_ratio"]), 95.0)),
...
def _plot_slices(slices: pd.DataFrame, out_path: Path) -> None:
    ...
    for mode_key in COMPARE_MODES:
        ...
        axes[0].plot(sub["D"], sub["slice_p95_abs_log_B_ratio"], marker="o", color=palette[mode_key], label=mode_key)
        axes[1].plot(sub["D"], sub["slice_p95_abs_delta_mu"], marker="o", color=palette[mode_key], label=mode_key)
        axes[2].plot(sub["D"], sub["slice_acceptance_mismatch"], marker="o", color=palette[mode_key], label=mode_key)
    axes[0].set_title("Layer-2 p95 |log B ratio|")
    axes[1].set_title("Layer-2 p95 |Δμ_μμ|")
    axes[2].set_title("Layer-2 acceptance mismatch")
    for ax in axes:
...
def main() -> None:
    ...
    detail_path = OUTDIR / "model_chain_decomposition_baseline_detail.csv"
    summary_path = OUTDIR / "model_chain_decomposition_baseline_summary.csv"
    slices_path = OUTDIR / "model_chain_decomposition_baseline_slices.csv"
    png_path = OUTDIR / "model_chain_decomposition_baseline.png"
    meta_path = OUTDIR / "model_chain_decomposition_baseline_run_meta.json"
