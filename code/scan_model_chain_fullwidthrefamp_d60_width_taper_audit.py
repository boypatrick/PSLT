...
def _build_candidate(alpha_center: float, taper: float, taper_center: float, taper_sigma: float):
    return make_baseline_kinetics(
        ...
        observable_ref_amp_anchor_center_D_override=float(REFAMP_CENTER_D),
        observable_ref_amp_anchor_sigma_D_override=float(REFAMP_SIGMA_D),
...
def _plot_heatmaps(summary: pd.DataFrame, out_path: Path) -> None:
    # Plot only the best sigma slice for each metric to keep the figure readable.
    ...
    for ax, (metric, title) in zip(
        ...
        best_sigma = (
            summary.groupby("taper_sigma_D", sort=True)[metric].min().sort_values().index[0]
        )
...
def main() -> None:
    ...
    detail_path = OUTDIR / "model_chain_fullwidthrefamp_d60_width_taper_audit_detail.csv"
    summary_path = OUTDIR / "model_chain_fullwidthrefamp_d60_width_taper_audit_summary.csv"
    slices_path = OUTDIR / "model_chain_fullwidthrefamp_d60_width_taper_audit_slices.csv"
    png_path = OUTDIR / "model_chain_fullwidthrefamp_d60_width_taper_audit.png"
    meta_path = OUTDIR / "model_chain_fullwidthrefamp_d60_width_taper_audit_run_meta.json"
