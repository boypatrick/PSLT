...
MODES = [
    ...
    ('cell_direct_runtime_release_fullwidthrefamp_partial2_pointamp2_widthboost', 'fullwidthrefamp_partial2_pointamp2_widthboost'),
]
...
def _summary_rows(detail: pd.DataFrame) -> pd.DataFrame:
    ...
    for mode_key, grp in sub.groupby('mode', sort=True):
        by_d = {float(d_val): float(np.max(np.abs(dsub['delta_mu']))) for d_val, dsub in grp.groupby('D', sort=True)}
        rows.append({
            'mode': mode_key,
...
def _plot_slices(slices: pd.DataFrame, out_path: Path) -> None:
    palette = {
        'fullwidthrefamp_pointamp2_widthboost': '#76b7b2',
        'fullwidthrefamp_partial2_pointamp2_widthboost': '#e15759',
    }
    fig, axes = plt.subplots(1, 2, figsize=(10.8, 4.2), constrained_layout=True, sharex=True)
    for mode_key in ['fullwidthrefamp_pointamp2_widthboost', 'fullwidthrefamp_partial2_pointamp2_widthboost']:
...
def main() -> None:
    ...
    detail_path = OUTDIR / 'model_chain_fullwidthrefamp_partial2_pointamp2_widthboost_hotspot_smoke_detail.csv'
    summary_path = OUTDIR / 'model_chain_fullwidthrefamp_partial2_pointamp2_widthboost_hotspot_smoke_summary.csv'
    slices_path = OUTDIR / 'model_chain_fullwidthrefamp_partial2_pointamp2_widthboost_hotspot_smoke_slices.csv'
    png_path = OUTDIR / 'model_chain_fullwidthrefamp_partial2_pointamp2_widthboost_hotspot_smoke.png'
    meta_path = OUTDIR / 'model_chain_fullwidthrefamp_partial2_pointamp2_widthboost_hotspot_smoke_run_meta.json'
