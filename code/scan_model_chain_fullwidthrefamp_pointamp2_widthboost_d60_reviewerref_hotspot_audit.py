            ...
            rows.append(
                {
                    "D": float(D),
                    "eta": float(eta),
                    "base_raw_amp": base["raw_amp"],
                    "base_raw_amp_ref": base["raw_amp_ref"],
                    "base_amp": base["amp"],
                    ...
                    "base_mu": base["mu"],
                    "base_chi2": base["chi2"],
                    "cur_raw_amp": cur["raw_amp"],
                    "cur_raw_amp_ref": cur["raw_amp_ref"],
                    "cur_amp": cur["amp"],
        ...
        rows.append(
            {
                ...
                "max_abs_delta_mu_width_pointamp_anchor": float(np.max(np.abs(grp["delta_mu_width_pointamp_anchor"]))),
...
def _plot(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    axes[1].plot(summary["D"], summary["p95_abs_delta_mu_width_pointamp_anchor"], marker="o", label="width+point-amp")
...
def main() -> None:
    ...
    detail_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp2_widthboost_d60_reviewerref_hotspot_audit_detail.csv"
    summary_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp2_widthboost_d60_reviewerref_hotspot_audit_summary.csv"
    slices_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp2_widthboost_d60_reviewerref_hotspot_audit_slices.csv"
    png_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp2_widthboost_d60_reviewerref_hotspot_audit.png"
    meta_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp2_widthboost_d60_reviewerref_hotspot_audit_run_meta.json"
