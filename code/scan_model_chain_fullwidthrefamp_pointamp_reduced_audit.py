...
MODES = [
    ...
    ("cell_direct_runtime_release_fullwidthrefamp_pointamp", "release_fullwidthrefamp_pointamp"),
    ("cell_direct_runtime_release_fullwidthrefamp_pointamp_widthboost", "release_fullwidthrefamp_pointamp_widthboost"),
]
...
def _effective_amp_pair(kin, layer: int, D: float, eta: float) -> tuple[float, float]:
    amp_raw = float(
    ...
    amp_ref = float(kin._blend_observable_ref_amp(amp_ref_raw, layer, OBSERVABLE_MODE, float(D)))
    amp = float(kin._blend_observable_point_amp(amp_raw, layer, OBSERVABLE_MODE, float(D), float(eta)))
    return amp, amp_ref
...
def _point_payload(kin, layer: int, D: float, eta: float) -> dict[str, float]:
    ...
    amp, amp_ref = _effective_amp_pair(kin, layer, D, eta)
...
def _plot_slices(slices: pd.DataFrame, out_path: Path) -> None:
    palette = {
        ...
        "release_fullwidthrefamp_pointamp": "#4e79a7",
        "release_fullwidthrefamp_pointamp_widthboost": "#f28e2b",
    }
...
def main() -> None:
    ...
    detail_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_reduced_audit_detail.csv"
    summary_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_reduced_audit_summary.csv"
    slices_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_reduced_audit_slices.csv"
    png_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_reduced_audit.png"
    meta_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_reduced_audit_run_meta.json"
