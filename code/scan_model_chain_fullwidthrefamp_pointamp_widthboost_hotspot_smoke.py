...
POINTAMP_PROFILE = OUTDIR / "model_chain_full_direct_point_amp_profile_D60_hotspot_layer2.csv"
...
MODES = [
    ...
    ("cell_direct_runtime_release_fullwidthrefamp_pointamp", "fullwidthrefamp_pointamp"),
    ("cell_direct_runtime_release_fullwidthrefamp_pointamp_widthboost", "fullwidthrefamp_pointamp_widthboost"),
]
...
def _plot_slices(slices: pd.DataFrame, out_path: Path) -> None:
    palette = {
        "fullwidthrefamp_pointamp": "#4e79a7",
        "fullwidthrefamp_pointamp_widthboost": "#f28e2b",
    }
    fig, axes = plt.subplots(1, 2, figsize=(10.5, 4.2), constrained_layout=True, sharex=True)
    for mode_key in ["fullwidthrefamp_pointamp", "fullwidthrefamp_pointamp_widthboost"]:
...
def main() -> None:
    ...
    if not POINTAMP_PROFILE.exists():
        raise FileNotFoundError(f"Missing point-amp profile: {POINTAMP_PROFILE}")
    kinetics = _build_kinetics()
    detail = _build_detail(kinetics)
    summary = _summary_rows(detail)
    slices = _slice_rows(detail)

    detail_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_widthboost_hotspot_smoke_detail.csv"
    summary_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_widthboost_hotspot_smoke_summary.csv"
    slices_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_widthboost_hotspot_smoke_slices.csv"
    png_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_widthboost_hotspot_smoke.png"
    meta_path = OUTDIR / "model_chain_fullwidthrefamp_pointamp_widthboost_hotspot_smoke_run_meta.json"
        ...
        json.dumps(
            {
                ...
                "point_amp_profile": str(POINTAMP_PROFILE),
            },
            indent=2,
        )
    )
