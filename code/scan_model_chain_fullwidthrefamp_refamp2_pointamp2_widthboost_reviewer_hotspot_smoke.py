...
FOCUS_D = np.array([float(D_GRID60[int(np.argmin(np.abs(D_GRID60 - target)))]) for target in FOCUS_TARGETS], dtype=float)
...
MODES = [
    ...
    ('cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost', 'base_mode'),
    ('cell_direct_runtime_release_fullwidthrefamp_refamp2_pointamp2_widthboost', 'refamp2_mode'),
]
...
def _summary_rows(detail: pd.DataFrame) -> pd.DataFrame:
    def lookup(by_d: dict[float, float], target: float) -> float:
        keys = np.array(list(by_d.keys()), dtype=float)
        if keys.size == 0:
            return float('nan')
        idx = int(np.argmin(np.abs(keys - float(target))))
        return float(by_d[float(keys[idx])])

    rows = []
    sub = detail[detail['mode'] != 'full_direct'].copy()
    for mode_key, grp in sub.groupby('mode', sort=True):
        by_d = {float(d): float(np.max(np.abs(g['delta_mu']))) for d, g in grp.groupby('D', sort=True)}
        rows.append({
            'mode': mode_key,
            ...
            'D5p627_max_abs_delta_mu': lookup(by_d, 5.627118644067797),
            'D5p898_max_abs_delta_mu': lookup(by_d, 5.898305084745763),
            'D6p169_max_abs_delta_mu': lookup(by_d, 6.169491525423729),
            'D6p441_max_abs_delta_mu': lookup(by_d, 6.4406779661016955),
            'D6p712_max_abs_delta_mu': lookup(by_d, 6.711864406779661),
        })
...
def _plot(slices: pd.DataFrame, out_path: Path) -> None:
    palette = {'base_mode': '#f28e2b', 'refamp2_mode': '#4e79a7'}
    fig, axes = plt.subplots(1, 2, figsize=(10.8, 4.2), constrained_layout=True, sharex=True)
    for mode_key in ['base_mode', 'refamp2_mode']:
    ...
    axes[0].set_title('reviewer-hotspot p95 |Δμ_μμ|')
    axes[1].set_title('reviewer-hotspot max |Δμ_μμ|')
    for ax in axes:
        ax.set_xlabel('D')
...
def main() -> None:
    ...
    detail_path = OUTDIR / 'model_chain_fullwidthrefamp_refamp2_pointamp2_widthboost_reviewer_hotspot_smoke_detail.csv'
    summary_path = OUTDIR / 'model_chain_fullwidthrefamp_refamp2_pointamp2_widthboost_reviewer_hotspot_smoke_summary.csv'
    slices_path = OUTDIR / 'model_chain_fullwidthrefamp_refamp2_pointamp2_widthboost_reviewer_hotspot_smoke_slices.csv'
    png_path = OUTDIR / 'model_chain_fullwidthrefamp_refamp2_pointamp2_widthboost_reviewer_hotspot_smoke.png'
    meta_path = OUTDIR / 'model_chain_fullwidthrefamp_refamp2_pointamp2_widthboost_reviewer_hotspot_smoke_run_meta.json'

    detail.to_csv(detail_path, index=False)
    summary.to_csv(summary_path, index=False)
    slices.to_csv(slices_path, index=False)
    _plot(slices, png_path)
    meta_path.write_text(json.dumps({'focus_D':[float(x) for x in FOCUS_D],'eta_grid':[float(x) for x in ETA_GRID],'ref_D':REF_D,'ref_eta':REF_ETA,'modes':[mode_key for _, mode_key in MODES]}, indent=2))
