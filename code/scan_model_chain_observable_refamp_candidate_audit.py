...
def _build_candidate(beta_peak: float, sigma_D: float):
    return make_baseline_kinetics(
        ...
        observable_ref_amp_anchor_peak_override=float(beta_peak),
        observable_ref_amp_anchor_center_D_override=5.6,
...
def _build_detail() -> pd.DataFrame:
    ...
    families = {(float(beta), float(sigma)): _build_candidate(float(beta), float(sigma)) for beta in BETAS for sigma in SIGMAS}
    rows: list[dict[str, float]] = []
    for D in FOCUS_D:
        for eta in ETA_GRID:
            base_mu, base_chi2 = _mu(kin_full, float(D), float(eta))
            for (beta_peak, sigma_D), kin in families.items():
...
def main() -> None:
    ...
    detail_path = OUTDIR / "model_chain_observable_refamp_candidate_audit_detail.csv"
    summary_path = OUTDIR / "model_chain_observable_refamp_candidate_audit_summary.csv"
    png_path = OUTDIR / "model_chain_observable_refamp_candidate_audit.png"
    meta_path = OUTDIR / "model_chain_observable_refamp_candidate_audit_run_meta.json"
    ...
    _write_mirror([detail_path, summary_path, png_path, meta_path])
