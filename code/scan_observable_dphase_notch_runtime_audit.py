...
def _notch_factor(D: float, eta: float, omega: float, m: float, phase: float, center_D: float, width_D: float, eta_couple: float) -> dict[str, float]:
    D_eff = float(D) / _gamma(BETA)
    gate = _gate(D_eff=D_eff, center_D=float(center_D), width_D=float(width_D))
    ...
    return {"D_eff": D_eff, "gate": gate, "notch": float(notch), "factor": factor}
...
def _counterfactual_mu(kin, D: float, eta: float, ref_D: float, ref_eta: float, mode: str, omega: float, m: float, phase: float, center_D: float, width_D: float, eta_couple: float, cache: dict) -> tuple[float, dict[str, float]]:
    base = _observable_payload(kin, D, eta, ref_D, ref_eta, cache)
    notch_payload = _notch_factor(
        D=float(D),
        eta=float(eta),
        omega=float(omega),
        m=float(m),
    ...
    factor = notch_payload["factor"]
    if mode == "partial_notch":
        partial_cf = float(base["partial_ratio"] * factor)
    elif mode == "pointamp_notch":
...
def _evaluate_combo(combo: dict[str, float | str], kin, region_tables: dict[str, pd.DataFrame], cache: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for region, cfg in REGIONS.items():
        ...
        for row in merged.itertuples(index=False):
            mu_cf, notch_payload = _counterfactual_mu(
                kin,
                D=float(row.D),
                eta=float(row.eta),
            ...
            detail.append(
                {
                    **combo,
                    ...
                    "D": float(row.D),
                    "eta": float(row.eta),
                    "D_eff": notch_payload["D_eff"],
                    "gate": notch_payload["gate"],
                    "notch": notch_payload["notch"],
                    "factor": notch_payload["factor"],
        ...
        rows.append(
            {
                ...
                "eta_couple": float(r60["eta_couple"]),
...
def _plot(summary_wide: pd.DataFrame, out_png: Path) -> None:
    ...
    modes = {"partial_notch": "#e15759", "pointamp_notch": "#4e79a7"}
...
def main() -> None:
    ...
    all_region_rows = []
    all_slice_rows = []
    total = len(MODE_GRID) * len(OMEGA_GRID) * len(M_GRID) * len(PHASE_GRID) * len(CENTER_GRID) * len(WIDTH_GRID) * len(ETA_COUPLE_GRID)
    combo_id = 0
                    ...
                    for center_D in CENTER_GRID:
                        for width_D in WIDTH_GRID:
                            for eta_couple in ETA_COUPLE_GRID:
                                ...
                                print(
                                    f"[combo {combo_id}/{total}] mode={mode} omega={omega} m={m} phase={phase:.3f} center={center_D} width={width_D} eta_couple={eta_couple}",
                                    flush=True,
                                )
        ...
        json.dumps(
            {
                ...
                "combos": combos,
                "notes": (
                    "Runtime-compatible observable-side D-phase notch shortlist on the current reviewer mainline. "
                    "The notch acts multiplicatively on the observable partial path or on the post-point-amplitude path, "
                    "using D_eff=D/gamma(beta) with beta fixed to the Lorentz witness value 0.4."
                ),
            },
            indent=2,
        )
