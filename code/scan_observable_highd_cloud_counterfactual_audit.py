...
STEM = "observable_highd_cloud_counterfactual_audit"

CHAIN_MODE = "cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost"
LAYER = 2

MODE_GRID = ("partial_cloud", "pointamp_cloud")
PEAK_GRID = (0.5, 1.0)
CENTER_GRID = (6.169492525423729, 6.4406779661016955)
WIDTH_GRID = (0.05, 0.12)
S_MIN_GRID = (0.60, 0.75)
POWER_GRID = (2.0,)
CLOUD_SIDE = 7
D_MIN = 4.0
D_MAX = 20.0
...
def _gate(D: float, peak: float, center: float, sigma: float) -> float:
    x = (float(D) - float(center)) / max(float(sigma), 1e-9)
    return float(peak) * float(np.exp(-0.5 * x * x))


def _cloud_scales(s_min: float, power: float) -> np.ndarray:
    ...
    return np.sqrt(np.outer(sigma, tau)).reshape(-1)
...
def _counterfactual_mu(kin, D: float, eta: float, ref_D: float, ref_eta: float, mode: str, peak: float, center_D: float, width_D: float, s_min: float, power: float, cache: dict) -> float:
    base = _observable_payload(kin, D, eta, ref_D, ref_eta, cache)
    alpha = _gate(D, peak=peak, center=center_D, sigma=width_D)
    if alpha <= 1e-12:
        return float(base["mu"])

    scales = _cloud_scales(s_min=s_min, power=power)
    local_amp = []
    local_partial = []
    for scale in scales:
        d_local = float(np.clip(float(D) * float(scale), D_MIN, D_MAX))
        payload = _observable_payload(kin, d_local, eta, ref_D, ref_eta, cache)
        local_amp.append(payload["amp_eff"])
        local_partial.append(payload["partial_ratio"])

    mean_amp = float(np.mean(local_amp))
    mean_partial = float(np.mean(local_partial))

    if mode == "pointamp_cloud":
        amp_cloud = (1.0 - alpha) * base["amp_eff"] + alpha * mean_amp
        partial_cloud = float((amp_cloud / max(base["amp_ref_eff"], 1e-30)) ** 2)
        partial_cloud = float(
            kin._blend_observable_partial_ratio(
                partial_ratio=partial_cloud,
                layer_n=int(LAYER),
                observable_mode=str(OBSERVABLE_MODE),
                D=float(D),
                eta=float(eta),
    ...
    elif mode == "partial_cloud":
        partial_cloud = float((1.0 - alpha) * base["partial_ratio"] + alpha * mean_partial)
    else:
        raise ValueError(f"unknown mode: {mode}")

    return float(partial_cloud / max(base["width_ratio"], 1e-30))
        ...
        rows.append(
            {
                ...
                "joint_objective": float(r60["p95_abs_delta_cf"] + 8.0 * max(float(r21["p95_abs_delta_cf"]) - float(r21["p95_abs_delta_base"]), 0.0)),
            }
        )
...
def _plot(summary_wide: pd.DataFrame, out_png: Path) -> None:
    ...
    modes = {"partial_cloud": "#e15759", "pointamp_cloud": "#4e79a7"}
...
def main() -> None:
    ...
    all_region_rows = []
    all_slice_rows = []
    total = len(MODE_GRID) * len(PEAK_GRID) * len(CENTER_GRID) * len(WIDTH_GRID) * len(S_MIN_GRID) * len(POWER_GRID)
    combo_id = 0
                ...
                for width_D in WIDTH_GRID:
                    for s_min in S_MIN_GRID:
                        for power in POWER_GRID:
                            ...
                            print(
                                f"[combo {combo_id}/{total}] mode={mode} peak={peak} center={center_D} width={width_D} s_min={s_min} power={power}",
                                flush=True,
                            )
        ...
        json.dumps(
            {
                ...
                "notes": (
                    "Counterfactual high-D cloud audit on current reviewer mainline. The cloud acts only on the observable point-amplitude or partial path, "
                    "using localized Gaussian gates and parabolic-style local D averaging. Width and Gamma_ref remain unchanged."
                ),
            },
            indent=2,
        )
