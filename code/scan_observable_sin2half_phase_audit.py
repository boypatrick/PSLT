...
STEM = "observable_sin2half_phase_audit"
...
KERNEL_GRID = ("current_cos_notch", "sin2_half_plus_phase")
...
def _phase_basis(kernel: str, theta_x: float, theta_y: float, phase: float) -> float:
    if kernel == "current_cos_notch":
        return float(0.5 * (1.0 - np.cos(theta_x)))
    if kernel == "sin2_half_plus_phase":
        return float(np.sin(0.5 * theta_x + 0.5 * float(phase)) ** 2)
...
def _two_lobe_factor(
    ...
    basis = _phase_basis(str(kernel), theta_x, theta_y, float(phase))
        ...
        json.dumps(
            {
                "chain_mode": CHAIN_MODE,
                "notes": (
                    "Controlled phase-basis audit comparing the current cos-notch control against a "
                    "sin^2(theta/2 + phi) kernel. The control isolates whether the new phase-shifted "
                    "trig basis adds useful D structure or is just a reparameterized version of the "
                    "current notch."
                ),
                "fixed_params": {
                    "beta": BETA,
