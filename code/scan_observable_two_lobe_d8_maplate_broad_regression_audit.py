...
STEM = 'observable_two_lobe_d8_maplate_broad_regression_audit'
...
CASE_CONFIG = {
    'D60E21_fullmap': {
        ...
        'candidate_map': MAP_DIR / 'hll_signal_strength_map_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_d8maplate_D60E21_runtime_20260329_refsnapfix_d8maplate_r3.csv',
    },
    'D21E41_fullmap': {
        ...
        'candidate_map': MAP_DIR / 'hll_signal_strength_map_chain_mode_cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_d8maplate_D21E41_runtime_20260329_refsnapfix_d8maplate_r3.csv',
    },
}


def _load_case(cfg: dict[str, object]) -> pd.DataFrame:
    ...
    runtime_df = pd.read_csv(cfg['runtime_map']).rename(columns={'mu_mumu': 'mu_runtime', 'chi2_mumu': 'chi2_runtime'})
    cand_df = pd.read_csv(cfg['candidate_map']).rename(columns={'mu_mumu': 'mu_cf', 'chi2_mumu': 'chi2_cf'})
    merged = (
    ...
    merged['worsened_vs_runtime'] = (merged['abs_delta_cf'] > merged['abs_delta_runtime']).astype(float)
...
def _subset_mask(df: pd.DataFrame, targets: list[float], subset: str) -> pd.Series:
    ...
    for target in targets:
        target_mask |= np.isclose(dvals, float(target), atol=1e-9)
...
def _summarize_subset(case_name: str, subset: str, df: pd.DataFrame) -> dict[str, float | str | int]:
    worst_idx = df['abs_improvement_vs_runtime'].idxmin()
    best_idx = df['abs_improvement_vs_runtime'].idxmax()
    ...
    return {
        ...
        'p95_abs_delta_runtime': float(np.percentile(df['abs_delta_runtime'], 95.0)),
        'p95_abs_delta_cf': float(np.percentile(df['abs_delta_cf'], 95.0)),
        'max_abs_delta_base': float(df['abs_delta_base'].max()),
        'max_abs_delta_runtime': float(df['abs_delta_runtime'].max()),
        'max_abs_delta_cf': float(df['abs_delta_cf'].max()),
        'mean_abs_improvement_vs_base': float(df['abs_improvement_vs_base'].mean()),
        'mean_abs_improvement_vs_runtime': float(df['abs_improvement_vs_runtime'].mean()),
        'improved_fraction_vs_base': float(df['improved_vs_base'].mean()),
        'improved_fraction_vs_runtime': float(df['improved_vs_runtime'].mean()),
        'worsened_fraction_vs_runtime': float(df['worsened_vs_runtime'].mean()),
        ...
        'best_point_improvement_vs_runtime': float(best['abs_improvement_vs_runtime']),
        'worst_point_D': float(worst['D']),
        'worst_point_eta': float(worst['eta']),
        'worst_point_improvement_vs_runtime': float(worst['abs_improvement_vs_runtime']),
    }
...
def _slice_rows(case_name: str, targets: list[float], df: pd.DataFrame) -> pd.DataFrame:
    ...
    for d_val, grp in df.groupby('D', sort=True):
        ...
        rows.append({
            'case': case_name,
            'D': d_float,
            'is_reviewer_target': float(any(np.isclose(d_float, t, atol=1e-9) for t in target_set)),
            ...
            'slice_max_abs_delta_runtime': float(grp['abs_delta_runtime'].max()),
            'slice_max_abs_delta_cf': float(grp['abs_delta_cf'].max()),
            'slice_mean_abs_improvement_vs_runtime': float(grp['abs_improvement_vs_runtime'].mean()),
            'slice_improved_fraction_vs_runtime': float(grp['improved_vs_runtime'].mean()),
            'slice_worsened_fraction_vs_runtime': float(grp['worsened_vs_runtime'].mean()),
...
def _plot(slices: pd.DataFrame, out_png: Path) -> None:
    ...
    axes = axes.flatten()
    palette = {'base': '#4e79a7', 'runtime': '#e15759', 'candidate': '#59a14f'}
    for ax, (case_name, grp) in zip(axes, slices.groupby('case', sort=False)):
        ...
        ax.plot(grp['D'], grp['slice_p95_abs_delta_runtime'], marker='o', color=palette['runtime'], label='raw twolobe')
        ax.plot(grp['D'], grp['slice_p95_abs_delta_cf'], marker='o', color=palette['candidate'], label='d8maplate')
        targets = grp.loc[grp['is_reviewer_target'] > 0.5, 'D'].tolist()
        for d in targets:
...
def main() -> None:
    ...
    for case_name, cfg in CASE_CONFIG.items():
        ...
        detail = _load_case(cfg)
        detail['case'] = case_name
        detail['is_reviewer_target'] = False
        for target in targets:
            detail.loc[np.isclose(detail['D'].to_numpy(dtype=float), float(target), atol=1e-9), 'is_reviewer_target'] = True
    ...
    meta_path.write_text(json.dumps({
        ...
        'notes': 'Broad full-map regression check for twolobe_d8maplate against current mainline base and corrected raw twolobe, including reviewer-target and non-reviewer complement subsets.'
    }, indent=2))
