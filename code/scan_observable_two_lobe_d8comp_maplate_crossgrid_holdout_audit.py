...
STEM = "observable_two_lobe_d8comp_maplate_crossgrid_holdout_audit"
...
TAG_SUFFIX = "20260330_crossgrid_holdout_r1"
...
def _load_case(case_name: str, cfg: dict[str, object], paths: dict[str, Path]) -> pd.DataFrame:
...
def main() -> None:
    ...
    detail_frames = []
    for case_name, cfg in CASE_CONFIG.items():
        targets = REGIONS[str(cfg["region_key"])]["D_targets"]
        detail = _load_case(case_name, cfg, path_table[case_name])
    ...
    meta_path.write_text(json.dumps({
        ...
        "notes": "Cross-grid holdout promotion check for twolobe_d8compmaplate on D60xE41 and D21xE21, comparing against fresh full_direct, current mainline, and d8maplate parent maps.",
    }, indent=2))
