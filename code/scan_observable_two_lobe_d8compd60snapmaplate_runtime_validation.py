...
STEM = "observable_two_lobe_d8compd60snapmaplate_runtime_validation"
...
def _load_case(case_name: str, cfg: dict[str, object]) -> tuple[pd.DataFrame, float]:
    ...
    ancestor_df = pd.read_csv(cfg["ancestor"]).rename(columns={"mu_mumu": "mu_ancestor", "chi2_mumu": "chi2_ancestor"})
    ...
    if case_name.startswith(("D60", "D80", "D100")):
        ...
        merged["mu_cf"] = merged["mu_parent"]
        merged["chi2_cf"] = merged["chi2_parent"]
...
def _plot(slices: pd.DataFrame, out_png: Path) -> None:
    ...
    for ax, case_name in zip(axes, order):
        ...
        ax.plot(grp["D"], grp["slice_p95_abs_delta_cf"], marker="o", color=palette["candidate"], label="d60snap")
...
def main() -> None:
    ...
    for case_name, cfg in CASE_CONFIG.items():
        detail, drift_d = _load_case(case_name, cfg)
        ...
        json.dumps(
            {
                ...
                "notes": (
                    "Formal validation for the D60-only conditional snapped-strip descendant. "
                    "The candidate is identical to twolobe_d8compmaplate on D21-family grids and applies the "
                    "snapped D≈7.24 mu_log repair only on dense D60-style grids."
                ),
            },
            indent=2,
        )
