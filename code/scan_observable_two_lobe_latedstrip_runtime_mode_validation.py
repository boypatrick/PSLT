...
STEM = "observable_two_lobe_latedstrip_runtime_mode_validation"

CHAIN_MODE = "cell_direct_runtime_release_fullwidthrefamp_pointamp2_widthboost_twolobe_latedstrip"
...
def _plot(summary: pd.DataFrame, out_png: Path) -> None:
    ...
    axes[0].bar(x + width, summary["p95_abs_delta_cf"], width=width, label="late-strip runtime")
        ...
        json.dumps(
            {
                ...
                "baseline_maps": {
                    k: {"full": str(v["full_map"]), "base": str(v["base_map"]), "runtime": str(v["runtime_map"])}
                    for k, v in CASE_CONFIG.items()
                },
                "notes": (
                    "Validation of the late-strip experimental runtime two-lobe mode against refreshed reviewer baselines and the raw two-lobe runtime mode. "
                    "The late-strip patch applies a narrow D=6.4 observable-layer correction after the base/runtime partial-width construction so that the "
                    "runtime D60 gains are preserved while the D21 strip overshoot is pulled back late in the chain."
                ),
            },
            indent=2,
        )
