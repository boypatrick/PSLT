...
def _run_map(tag: str, d_num: int, eta_num: int) -> Path:
    env = os.environ.copy()
    ...
    return MAP_DIR / f"hll_signal_strength_map_{tag}.csv"


def _evaluate_region(region: str, cfg: dict[str, object], candidate_map: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    merged["abs_improvement"] = merged["abs_delta_base"] - merged["abs_delta_cf"]
    merged["cf_shift_vs_base"] = merged["mu_cf"] - merged["mu_base"]
    merged["improved"] = (merged["abs_delta_cf"] < merged["abs_delta_base"]).astype(float)
    merged["worsened"] = (merged["abs_delta_cf"] > merged["abs_delta_base"]).astype(float)
        ...
        [
            {
                ...
                "mean_abs_improvement": float(np.mean(merged["abs_improvement"])),
                "improved_fraction": float(np.mean(merged["improved"])),
                "worsened_fraction": float(np.mean(merged["worsened"])),
                "acceptance_flip_fraction": float(np.mean(merged["acceptance_flip_vs_base"])),
                "best_point_D": float(merged.loc[merged["abs_improvement"].idxmax(), "D"]),
                "best_point_eta": float(merged.loc[merged["abs_improvement"].idxmax(), "eta"]),
                "best_point_improvement": float(np.max(merged["abs_improvement"])),
                "worst_point_D": float(merged.loc[merged["abs_improvement"].idxmin(), "D"]),
                "worst_point_eta": float(merged.loc[merged["abs_improvement"].idxmin(), "eta"]),
                "worst_point_improvement": float(np.min(merged["abs_improvement"])),
            }
        ]
    )
...
def _plot(summary: pd.DataFrame, out_png: Path) -> None:
    ...
    width = 0.36
    axes[0].bar(x - width / 2.0, summary["p95_abs_delta_base"], width=width, label="base")
    axes[0].bar(x + width / 2.0, summary["p95_abs_delta_cf"], width=width, label="two-lobe runtime")
    axes[0].set_xticks(x, regions)
    axes[0].set_title("Reviewer p95 |Δμ|")
    axes[0].grid(True, alpha=0.25)
    axes[0].legend(frameon=False)

    axes[1].bar(x - width / 2.0, summary["improved_fraction"], width=width, label="improved")
    axes[1].bar(x + width / 2.0, summary["worsened_fraction"], width=width, label="worsened")
    axes[1].set_xticks(x, regions)
    axes[1].set_ylim(0.0, 1.0)
    axes[1].set_title("Fraction vs base")
...
def main() -> None:
    ...
    candidate_maps = {
        region: _run_map(str(cfg["tag"]), int(cfg["d_num"]), int(cfg["eta_num"]))
    ...
    detail_rows = []
    for region, cfg in CASE_CONFIG.items():
        region_summary, region_slices, detail = _evaluate_region(region, cfg, candidate_maps[region])
        ...
        json.dumps(
            {
                ...
                "baseline_maps": {k: {"full": str(v["full_map"]), "base": str(v["base_map"])} for k, v in CASE_CONFIG.items()},
                "notes": (
                    "Validation of the experimental runtime two-lobe D-phase notch mode against refreshed reviewer baselines. "
                    "The mode uses the best joint shortlist parameters from the recentered two-lobe audit."
                ),
            },
            indent=2,
        )
