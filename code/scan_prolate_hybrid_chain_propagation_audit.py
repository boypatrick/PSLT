...
from scan_prolate_gn_chi_compatibility_audit import HOTSPOT_BAND, KEY_D, _load_witness, _solve_profiles


ROOT = Path(__file__).resolve().parent.parent
OUTDIR = ROOT / "output" / "kinetic_action_chain"
PAPER_DIR = ROOT / "paper"
STEM = "prolate_hybrid_chain_propagation_audit"
...
COMPARE_PAIRS = (
    ...
    ("hybrid_vs_uniform", "hybrid_profile", "uniform_profile"),
    ("hybrid_vs_runtime", "hybrid_profile", "runtime_mainline"),
)
...
def _build_hybrid_profiles() -> tuple[pd.DataFrame, Path, Path, Path]:
    ...
    chi_hybrid = _anchor_to_reference(d_vals, detail["prolate_gap_fraction"], detail["chi_LR"])
    g2_hybrid = _anchor_to_reference(d_vals, detail["prolate_omega2"], detail["g2_raw"])
    g3_hybrid = _anchor_to_reference(d_vals, detail["prolate_omega2"], detail["g3_raw"])

    detail["chi_LR_hybrid"] = chi_hybrid
    detail["g2_raw_hybrid"] = g2_hybrid
    ...
    gn_base = gn_base.merge(detail[["D", "g2_raw_hybrid", "g3_raw_hybrid"]], on="D", how="inner")
    gn_base["g2_raw"] = gn_base["g2_raw_hybrid"]
    ...
    gn_base["g3_hat"] = 1.0
    gn_base = gn_base.drop(columns=["g2_raw_hybrid", "g3_raw_hybrid"])

    scale_df = detail[["D", "g2_raw", "g3_raw", "g2_raw_hybrid", "g3_raw_hybrid"]].copy()
    scale_df["scale2"] = scale_df["g2_raw_hybrid"] / np.maximum(scale_df["g2_raw"], 1e-30)
    ...
    spec_base = spec_base.merge(scale_df[["D", "scale2", "scale3", "g3_raw_hybrid"]], on="D", how="inner")
    ...
    spec_base = spec_base.drop(columns=["scale2", "scale3", "g3_raw_hybrid"])

    chi_profile = detail[["D", "chi_LR", "chi_LR_hybrid", "Gamma_ref"]].copy()
...
def _build_kinetics(chi_path: Path, gn_path: Path, spec_path: Path, hybrid_detail: pd.DataFrame) -> dict[str, PSLTKinetics]:
    ...
    return {
        ...
        "hybrid_profile": PSLTKinetics(params=hybrid_params),
    }
...
def main() -> None:
        ...
        json.dumps(
            {
                ...
                "hybrid_assignment": {
                    "chi_LR": "prolate_gap_fraction anchored at D_ref",
                    "g2_raw": "prolate_omega2 anchored at D_ref",
                    "g3_raw": "prolate_omega2 anchored at D_ref",
                    "Gamma_ref": "uniform baseline retained",
                },
                "generated_profiles": {
    ...
    for path in [chi_path, gn_path, spec_path, detail_path, summary_path, slices_path, png_path, meta_path]:
