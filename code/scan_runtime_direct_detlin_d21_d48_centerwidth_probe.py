...
TAG = os.environ.get("D21_D48_CENTERWIDTH_TAG", "v1_3x3").strip() or "v1_3x3"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d21_d48_centerwidth_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d21_d48_centerwidth_probe_{TAG}_summary.json"

CENTERS = [float(x) for x in os.environ.get("CENTERS", "4.7,4.8,4.9").split(",") if x.strip()]
WIDTHS = [float(x) for x in os.environ.get("WIDTHS", "0.35,0.40,0.45").split(",") if x.strip()]
TARGET_D = 4.8
FIXED_BETA = float(os.environ.get("D48_BETA_D21_FIXED", "0.69"))
...
ENV_CONST = {
    ...
    "STRICT_D48_BETA_D21": str(FIXED_BETA),
...
def main() -> None:
    ...
    for center in CENTERS:
        for width in WIDTHS:
            i += 1
            mod = load_validator()
            # Override only the D=4.8 width-band geometry for this probe run.
            mod.WIDTH_BANDS[1]["center"] = float(center)
            mod.WIDTH_BANDS[1]["half_width"] = float(width)
            ...
            result = {
                "d48_center_d21": float(center),
                "d48_half_width_d21": float(width),
                "d48_beta_d21": float(FIXED_BETA),
            ...
            print(
                ...
                f"delta_vs_v25={result['delta_p95_vs_v25']:+.6f} "
