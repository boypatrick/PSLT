...
BASELINE_DETAIL = Path(
    os.environ.get(
        "BASELINE_DETAIL",
        str(ROOT / "output" / "kinetic_action_chain" / "runtime_direct_detlin_generalization_fast_detail_gridaware_rule_d40d64d48d64amp_v25.csv"),
    )
)
TAG = os.environ.get("D21_D48_AMP_PROBE_TAG", "v1").strip() or "v1"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d21_d48_local_amp_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d21_d48_local_amp_probe_{TAG}_summary.json"
...
TARGET_CENTER = float(os.environ.get("TARGET_CENTER", "4.8"))
TARGET_HALF_WIDTH = float(os.environ.get("TARGET_HALF_WIDTH", "0.40"))
ALPHAS = [float(x) for x in os.environ.get("ALPHAS", "0.00,0.01,0.02,0.03,0.04").split(",") if x.strip()]


def _patch_width_bands(kin, alpha: float) -> None:
    base_bands = [
        {"beta": D40_BETA_D21, "center": 4.0, "half_width": 0.40},
        {"beta": D48_BETA_D21, "center": 4.8, "half_width": 0.40},
        {"beta": D72_BETA_D21, "center": 7.2, "half_width": 0.40},
        {"beta": D80_BETA_D21, "center": 8.0, "half_width": 0.40},
    ]
...
def _patch_gnorm(kin) -> None:
    def patched_gnorm(self, hll_diag, direct_diag, direct_b123):
        ...
        activation = float(max(0.0, 1.0 - abs(metric - D21_G_CENTER) / D21_G_HALF_WIDTH))
        ...
        return float(D21_G_BETA * activation * b2_gate)
