...
BASELINE_DETAIL = Path(
    os.environ.get(
        ...
        str(
            ...
            / "runtime_direct_detlin_generalization_fast_detail_gridaware_rule_d40d64d48d64amp_v16.csv"
        ),
    )
)
TAG = os.environ.get("D21_D64_CENTERWIDTH_TAG", "v1_3x3").strip() or "v1_3x3"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d21_d64_centerwidth_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d21_d64_centerwidth_probe_{TAG}_summary.json"

CENTERS = [float(x) for x in os.environ.get("CENTERS", "0.06,0.065,0.07").split(",") if x.strip()]
WIDTHS = [float(x) for x in os.environ.get("WIDTHS", "0.04,0.045,0.05").split(",") if x.strip()]
TARGET_D = 6.4
FIXED_BETA = float(os.environ.get("GNORM_BETA_D21_FIXED", "0.31"))

ENV_CONST = {
    "STRICT_GNORM_BETA_D21": str(FIXED_BETA),
...
def main() -> None:
    ...
    rows = []
    total = len(CENTERS) * len(WIDTHS)
    i = 0
    for center in CENTERS:
        for width in WIDTHS:
            i += 1
            os.environ["STRICT_GNORM_CENTER_D21"] = str(center)
            os.environ["STRICT_GNORM_HALF_WIDTH_D21"] = str(width)
            mod = load_validator()
            ...
            result = {
                "g_center_d21": float(center),
                "g_half_width_d21": float(width),
                "g_beta_d21": float(FIXED_BETA),
                ...
                "delta_p95_vs_v16": float(np.percentile(deltas, 95.0) - baseline_p95),
                "delta_max_vs_v16": float(np.max(deltas) - baseline_max),
            }
            rows.append(result)
            ...
            print(
                f"[progress] {i}/{total} center={center:.3f} width={width:.3f} "
                f"p95={result['p95_abs_delta_mu_mumu']:.6f} "
                f"delta_vs_v16={result['delta_p95_vs_v16']:+.6f} "
