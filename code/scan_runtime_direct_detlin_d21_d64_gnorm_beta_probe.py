...
BASELINE_DETAIL = Path(
    os.environ.get(
        "BASELINE_DETAIL",
        str(ROOT / "output" / "kinetic_action_chain" / "runtime_direct_detlin_generalization_fast_detail_gridaware_rule_d40d64d48d64amp_v15.csv"),
    )
)
TAG = os.environ.get("D21_D64_PROBE_TAG", "v1").strip() or "v1"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d21_d64_gnorm_beta_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d21_d64_gnorm_beta_probe_{TAG}_summary.json"

BETAS = [float(x) for x in os.environ.get("BETAS", "0.24,0.27,0.30,0.33,0.36").split(",") if x.strip()]
TARGET_D = 6.4
...
def main() -> None:
    ...
    for i, beta in enumerate(BETAS, start=1):
        os.environ["STRICT_GNORM_BETA_D21"] = str(beta)
        mod = load_validator()
        ...
        result = {
            ...
            "delta_p95_vs_v15": float(np.percentile(deltas, 95.0) - baseline_p95),
            "delta_max_vs_v15": float(np.max(deltas) - baseline_max),
        }
        rows.append(result)
        ...
        print(
            ...
            f"delta_vs_v15={result['delta_p95_vs_v15']:+.6f} "
