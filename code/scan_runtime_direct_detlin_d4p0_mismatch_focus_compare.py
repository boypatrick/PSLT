...
OUT_SUMMARY = OUTDIR / "runtime_direct_detlin_d4p0_mismatch_focus_compare_summary.csv"
OUT_DETAIL = OUTDIR / "runtime_direct_detlin_d4p0_mismatch_focus_compare_detail.csv"
OUT_PNG = OUTDIR / "runtime_direct_detlin_d4p0_mismatch_focus_compare.png"
OUT_META = OUTDIR / "runtime_direct_detlin_d4p0_mismatch_focus_compare_run_meta.json"
...
D4P8_WIDTH = {"beta": 0.40, "center": 4.8, "half_width": 0.40}
D4P0_WIDTH = {
    "beta": float(os.environ.get("D4P0_BETA", "1.00")),
    "center": 4.0,
    "half_width": 0.40,
}
...
def _patch_width_bands(kin, *, include_d4p0: bool) -> None:
...
def _evaluate(label: str, *, include_d4p0: bool, full_map, eta_grid, mu_obs: float, sigma_obs: float) -> pd.DataFrame:
    kin = _build_kinetics()
    _set_width_fix(kin)
    _patch_gnorm_bandpass(kin)
    _patch_width_bands(kin, include_d4p0=include_d4p0)
...
def main() -> None:
    ...
    base_df = _evaluate("baseline_d4p8_only", include_d4p0=False, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    cand_df = _evaluate("candidate_d4p0_beta1p00", include_d4p0=True, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    ...
    ax.plot(summary_df["D"], summary_df["baseline_p95_abs_delta_mu_mumu"], marker="o", linewidth=2.0, label="baseline_d4p8_only")
    ax.plot(summary_df["D"], summary_df["candidate_p95_abs_delta_mu_mumu"], marker="s", linewidth=2.0, label="candidate_d4p0_beta1p00")
    ax.set_xlabel("D")
    ax.set_ylabel("p95 |Δmu_mumu| across eta")
    ax.set_title("strict all-direct D=4.0 mismatch exact compare")
    ax.grid(alpha=0.25)
    ...
    OUT_META.write_text(json.dumps({"focus_D": FOCUS_D, "eta_grid": eta_grid, "gnorm_bandpass": GNORM_BANDPASS, "d4p8_width": D4P8_WIDTH, "d4p0_width": D4P0_WIDTH, "width_fix": WIDTH_FIX}, indent=2))
