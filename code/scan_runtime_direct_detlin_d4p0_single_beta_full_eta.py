...
OUT_PREFIX = "runtime_direct_detlin_d4p0_single_beta_full_eta"
...
D4P0_WIDTH = {
    "beta": float(os.environ.get("D4P0_BETA", "2.00")),
    "center": 4.0,
    "half_width": 0.40,
}
...
def _evaluate(label: str, *, with_d4p0_width: bool, full_map, eta_grid, mu_obs: float, sigma_obs: float) -> pd.DataFrame:
    kin = _build_kinetics()
    _set_width_fix(kin)
    ...
    if with_d4p0_width:
        ...
        # preserve D4.8 route even in baseline
        original = kin._blend_observable_width_ratio
...
def main() -> None:
    ...
    out_summary = OUTDIR / f"{OUT_PREFIX}_summary_beta{str(D4P0_WIDTH['beta']).replace('.', 'p')}.csv"
    out_detail = OUTDIR / f"{OUT_PREFIX}_detail_beta{str(D4P0_WIDTH['beta']).replace('.', 'p')}.csv"
    out_png = OUTDIR / f"{OUT_PREFIX}_beta{str(D4P0_WIDTH['beta']).replace('.', 'p')}.png"
    out_meta = OUTDIR / f"{OUT_PREFIX}_run_meta_beta{str(D4P0_WIDTH['beta']).replace('.', 'p')}.json"
    ...
    base_df = _evaluate("baseline_d4p8_only", with_d4p0_width=False, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    cand_df = _evaluate(f"candidate_d4p0_beta{str(D4P0_WIDTH['beta']).replace('.', 'p')}", with_d4p0_width=True, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    detail_df = pd.concat([base_df, cand_df], ignore_index=True)

    summary_df = pd.DataFrame(
        [
            {
                "d4p0_beta": float(D4P0_WIDTH["beta"]),
    ...
    axes[0].set_title(f"D=4.0 full-eta compare (beta={D4P0_WIDTH['beta']:.2f})")
    ...
    axes[1].axhline(4.0, color="black", linestyle="--", linewidth=1.2, label="chi2=4")
    axes[1].set_xlabel("eta")
    axes[1].set_ylabel("chi2_mumu")
    axes[1].set_title("Acceptance-side compare")
