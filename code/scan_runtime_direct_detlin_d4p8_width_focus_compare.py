...
OUT_SUMMARY = OUTDIR / "runtime_direct_detlin_d4p8_width_focus_compare_summary.csv"
OUT_DETAIL = OUTDIR / "runtime_direct_detlin_d4p8_width_focus_compare_detail.csv"
OUT_PNG = OUTDIR / "runtime_direct_detlin_d4p8_width_focus_compare.png"
OUT_META = OUTDIR / "runtime_direct_detlin_d4p8_width_focus_compare_run_meta.json"
...
WIDTH_PROBE = {"label": "width_probe_beta0p40_hw0p40", "width_beta": 0.40, "width_center": 4.8, "width_half_width": 0.40}
...
def _patch_width_bandpass(kin) -> None:
    original = kin._blend_observable_width_ratio
    beta = float(WIDTH_PROBE["width_beta"])
    center = float(WIDTH_PROBE["width_center"])
    half_width = float(WIDTH_PROBE["width_half_width"])
    ...
    def patched(self, width_ratio: float, D: float, eta: float) -> float:
        ...
        activation = float(max(0.0, 1.0 - abs(float(D) - center) / half_width))
        positive_log_width = float(max(np.log(max(base, floor)), 0.0))
        return float(np.exp(np.log(max(base, floor)) + beta * activation * positive_log_width))

    kin._blend_observable_width_ratio = types.MethodType(patched, kin)


def _evaluate(label: str, *, with_width_probe: bool, full_map, eta_grid, mu_obs: float, sigma_obs: float) -> pd.DataFrame:
    kin = _build_kinetics()
    _set_width_fix(kin)
    _patch_gnorm_bandpass(kin)
    if with_width_probe:
        _patch_width_bandpass(kin)
...
def main() -> None:
    ...
    base_df = _evaluate("gnorm_bandpass_only", with_width_probe=False, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    cand_df = _evaluate(str(WIDTH_PROBE["label"]), with_width_probe=True, full_map=full_map, eta_grid=eta_grid, mu_obs=mu_obs, sigma_obs=sigma_obs)
    ...
    ax.plot(summary_df["D"], summary_df["baseline_p95_abs_delta_mu_mumu"], marker="o", linewidth=2.0, label="gnorm_bandpass_only")
    ax.plot(summary_df["D"], summary_df["candidate_p95_abs_delta_mu_mumu"], marker="s", linewidth=2.0, label=str(WIDTH_PROBE["label"]))
    ax.set_xlabel("D")
    ax.set_ylabel("p95 |Δmu_mumu| across eta")
    ax.set_title("strict all-direct D=4.8 width probe exact compare")
    ax.grid(alpha=0.25)
    ...
    OUT_META.write_text(json.dumps({"focus_D": FOCUS_D, "eta_grid": eta_grid, "gnorm_bandpass": GNORM_BANDPASS, "width_probe": WIDTH_PROBE, "width_fix": WIDTH_FIX}, indent=2))
