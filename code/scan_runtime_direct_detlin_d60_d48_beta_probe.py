...
BASELINE_DETAIL = Path(
    os.environ.get(
        ...
        str(
            ...
            / "runtime_direct_detlin_generalization_fast_detail_gridaware_rule_d40d64d48d64amp_v17.csv"
        ),
    )
)
TAG = os.environ.get("D60_D48_PROBE_TAG", "v1").strip() or "v1"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d60_d48_beta_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d60_d48_beta_probe_{TAG}_summary.json"

BETAS = [float(x) for x in os.environ.get("BETAS", "0.45,0.50,0.55,0.60,0.65").split(",") if x.strip()]
TARGET_D = 4.8
...
def main() -> None:
    ...
    for i, beta in enumerate(BETAS, start=1):
        os.environ["STRICT_D48_BETA_D60"] = str(beta)
        mod = load_validator()
        kin = mod.build_kinetics("D60E21_release", float(df["D"].min()), float(df["D"].max()), int(df["D"].nunique()))
        ...
        result = {
            ...
            "delta_p95_vs_v17": float(np.percentile(deltas, 95.0) - baseline_p95),
            "delta_max_vs_v17": float(np.max(deltas) - baseline_max),
        }
        rows.append(result)
        ...
        print(
            ...
            f"delta_vs_v17={result['delta_p95_vs_v17']:+.6f} "
