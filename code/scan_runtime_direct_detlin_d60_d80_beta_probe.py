...
BASELINE_DETAIL = Path(
    os.environ.get(
        ...
        str(
            ...
            / "runtime_direct_detlin_generalization_fast_detail_gridaware_rule_d40d64d48d64amp_v22.csv"
        ),
    )
)
TAG = os.environ.get("D60_D80_PROBE_TAG", "v1").strip() or "v1"
OUT_SAMPLES = OUTDIR / f"runtime_direct_detlin_d60_d80_beta_probe_{TAG}_samples.csv"
OUT_SUMMARY = OUTDIR / f"runtime_direct_detlin_d60_d80_beta_probe_{TAG}_summary.json"

BETAS = [float(x) for x in os.environ.get("BETAS", "1.00,1.10,1.20,1.30,1.40").split(",") if x.strip()]
TARGET_D = 8.0
...
def main() -> None:
    ...
    for i, beta in enumerate(BETAS, start=1):
        mod = load_validator()
        mod.WIDTH_BANDS[4]["beta"] = float(beta)
        ...
        result = {
            ...
            "delta_p95_vs_v22": float(np.percentile(deltas, 95.0) - baseline_p95),
            "delta_max_vs_v22": float(np.max(deltas) - baseline_max),
        }
        rows.append(result)
        ...
        print(
            ...
            f"delta_vs_v22={result['delta_p95_vs_v22']:+.6f} "
