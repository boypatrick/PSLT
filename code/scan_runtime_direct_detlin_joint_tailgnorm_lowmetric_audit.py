...
OUT_SCAN = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit_scan.csv"
OUT_SUMMARY = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit_summary.csv"
OUT_SLICES = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit_best_slices.csv"
OUT_ACTIVATION = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit_best_activation.csv"
OUT_PNG = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit.png"
OUT_META = OUTDIR / "runtime_direct_detlin_joint_tailgnorm_lowmetric_audit_run_meta.json"
...
# Reuse the strongest width-only detlin extension and only test whether a
# narrow direct-gnorm gate can rescue the D=6.4 UV-dominated strip without
# reopening D=4.8/7.2/8.0.
WIDTH_FIX = {
...
BETA_VALUES = [0.02, 0.04, 0.06, 0.08, 0.10, 0.12, 0.16, 0.20]
...
def _set_candidate_params(kin, beta: float, metric_lo: float, metric_hi: float) -> None:
    params = kin.params
    ...
    params.hll_uv_runtime_direct_gnorm_blend_max = float(beta)
    params.hll_uv_runtime_direct_gnorm_metric_lo = float(metric_lo)
    params.hll_uv_runtime_direct_gnorm_metric_hi = float(metric_hi)
...
def _evaluate_candidate(
    ...
    *,
    ...
    _set_candidate_params(kin, beta=beta, metric_lo=metric_lo, metric_hi=metric_hi)
            ...
            point_rows.append(
                {
                    "label": label,
                    "beta": float(beta),
                    "metric_lo": float(metric_lo),
                    "metric_hi": float(metric_hi),
                    "D": float(D),
                    "eta": float(eta),
...
def _activation_diag(kin, D: float) -> dict[str, float]:
    direct = kin._runtime_direct_b_operator_inputs(float(D))
    floor = float(kin.params.hll_uv_coupling_floor)
    # Use the same pre-activation diagonals as the runtime gnorm gate.
    ...
    lo = float(kin.params.hll_uv_runtime_direct_gnorm_metric_lo)
    hi = float(kin.params.hll_uv_runtime_direct_gnorm_metric_hi)
    if hi <= lo + 1e-12:
        activation = 1.0 if metric > lo else 0.0
    else:
        activation = float(np.clip((metric - lo) / (hi - lo), 0.0, 1.0))
    weight = float(kin.params.hll_uv_runtime_direct_gnorm_blend_max) * activation * b2_gate
    return {
        "D": float(D),
        "metric": float(metric),
...
def main() -> None:
    ...
    candidates = [
        {"label": "width_only", "beta": 0.0, "metric_lo": 0.0, "metric_hi": 1.0},
    ]
    for beta in BETA_VALUES:
        for metric_lo, metric_hi in METRIC_WINDOWS:
            candidates.append(
                {
                    "label": "joint_narrow_gnorm",
                    "beta": float(beta),
                    "metric_lo": float(metric_lo),
                    "metric_hi": float(metric_hi),
                }
    ...
    for cand in candidates:
        ...
        row, points = _evaluate_candidate(
            ...
            metric_lo=float(cand["metric_lo"]),
            metric_hi=float(cand["metric_hi"]),
        )
        rows.append(row)
        point_rows.extend(points)

    scan_df = pd.DataFrame(rows).sort_values(
        ["objective", "d6p4_max_abs_delta_mu_mumu", "d4p8_max_abs_delta_mu_mumu", "beta", "metric_lo", "metric_hi"]
    ...
    best_points = points_df[
        ...
        & (np.isclose(points_df["metric_lo"], float(best["metric_lo"])))
        & (np.isclose(points_df["metric_hi"], float(best["metric_hi"])))
    ].copy()
        ...
        slice_rows.append(
            {
                ...
                "best_joint_p95_abs_delta_mu_mumu": float(np.percentile(best_sub["abs_delta_mu_mumu"], 95.0)),
                "best_joint_max_abs_delta_mu_mumu": float(best_sub["abs_delta_mu_mumu"].max()),
                "best_joint_acceptance_mismatch": float(best_sub["acceptance_mismatch"].mean()),
            }
        )
    slices_df = pd.DataFrame(slice_rows)

    kin = _build_kinetics()
    _set_candidate_params(
        kin,
        beta=float(best["beta"]),
        metric_lo=float(best["metric_lo"]),
        metric_hi=float(best["metric_hi"]),
    )
    activation_df = pd.DataFrame([_activation_diag(kin, D) for D in FOCUS_D])
    ...
    summary = {
        ...
        "num_improve_d6p4_vs_width_only": int(
            np.sum(scan_df["d6p4_max_abs_delta_mu_mumu"] < float(width_only["d6p4_max_abs_delta_mu_mumu"]) - 1e-12)
        ),
        "best_max_gnorm_weight_focus": float(activation_df["weight"].max()),
        "best_max_gnorm_metric_focus": float(activation_df["metric"].max()),
    }
    summary_df = pd.DataFrame([summary])

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.5), constrained_layout=True)

    joint_df = scan_df[scan_df["label"] == "joint_narrow_gnorm"].copy()
    sc = axes[0].scatter(
        joint_df["d4p8_max_abs_delta_mu_mumu"],
        joint_df["d6p4_max_abs_delta_mu_mumu"],
        c=joint_df["beta"],
        cmap="viridis",
        s=42,
        alpha=0.85,
    ...
    axes[0].set_xlabel("D=4.8 max |Δmu_mumu|")
    axes[0].set_ylabel("D=6.4 max |Δmu_mumu|")
    axes[0].set_title("width-only vs narrow gnorm tradeoff")
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc="upper right")
    cbar = fig.colorbar(sc, ax=axes[0], shrink=0.9)
    cbar.set_label("gnorm beta")
    ...
    axes[1].plot(
        slices_df["D"],
        slices_df["best_joint_p95_abs_delta_mu_mumu"],
        marker="s",
        linewidth=2.0,
        label="best joint",
    )
    ...
    fig.suptitle("strict all-direct joint tailm2 + low-metric gnorm audit")
