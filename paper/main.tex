...
We verify the closure against two benchmarks: (1) internal stability of the three-generation hierarchy (Generation Ratio $> 90\%$ over most sampled points), and (2) a proxy compatibility test with the ATLAS Run-3 $H\to\mu\mu$ signal strength ($\mu_{\mu\mu}^{\rm obs}=1.4\pm0.4$).
...
\midrule
 Kinetics $\Gamma_N$ & $\chi_N^{(LR)}(D)$ & App.~\ref{app:chi_loc} & Localized-channel profile (action-derived extraction, D-interpolated in scan) \\
...
\label{eq:R3}
\end{equation}
With $\chi_N^{(LR)}(D)$ interpolation from Appendix~\ref{app:chi_loc}, our numerical scan gives $\mathcal{R}_3 > 90\%$ over $80.0\%$ of the sampled $(D,\eta)$ grid, while no point reaches $\mathcal{R}_3 > 95\%$ in this setup. This still supports a dominant three-layer sector, but with a stricter robustness statement than earlier constant-$\chi$ scans. We emphasize that this global map uses a simplified surrogate kinetic operator, with only the mixing-profile input imported from the action-derived extraction.
...
\begin{enumerate}
    \item \textbf{Observable mapping}: the $H\to\mu\mu$ comparison uses the proxy $g_{h\mu\mu}^{\rm eff}\propto \sqrt{W_2}$ rather than a full derivation from the EYMH action.
