...
$g_N$ phase-space full-tail mode (\texttt{fp\_2d\_full}) with phase-space normalization & Baseline mode + legacy Cardy reference retained for comparator audits & Yes & Promote full $(D,\eta,N)$ action-derived extraction beyond map-level profile closure \\
...
\end{enumerate}
At the same time, the present implementation keeps two EFT-level approximations explicit: map-level profile closure in the $g_N(D)$ sector and a bounded nearest-neighbor flavor projector in the matched $H\to\mu\mu$ map of Eq.~\eqref{eq:mu_pred}.
