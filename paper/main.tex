...
Open-system $\chi_{\rm eff}$ (Lindblad; geometry + micro profiles) & Scan-ready diagnostic module with profiled $\gamma_\phi(D),\gamma_{\rm mix}(D)$, including anchor-calibrated \texttt{open\_system\_micro} profile (Appendix~\ref{app:chi_open_scan}) & Diagnostic (baseline-candidate gate audited; not promoted) & Derive microscopic EYMH bath $(L_k,\gamma_k)$ and then promote to baseline if robust \\
...
\end{equation}
via \texttt{extract\_chi\_open\_system\_micro.py}. To avoid map-level fitting, we calibrate $\kappa_{\rm env}$ with a fixed single-point anchor rule (\texttt{calibrate\_kappa\_env\_micro\_anchor.py}): match the geometry-profile ratio at $D_{\rm ref}=12$. This gives $\kappa_{\rm env}=2.36\times10^{5}$ with anchor mismatch $|\Delta{\rm ratio}|=1.92\times10^{-3}$. The corresponding scan-level band is
\begin{equation}
\chi_{\rm open}^{\rm(micro)}/\chi_{LR}\in[0.0096,0.2341],\qquad \langle \chi_{\rm open}^{\rm(micro)}/\chi_{LR}\rangle=0.0885,
...
open-micro base $(1,1)$ & 0.010 & 0.234 & 0.088 \\
$\phi\times0.5$, mix$\times1$ & 0.010 & 0.234 & 0.088 \\
$\phi\times2$, mix$\times1$ & 0.010 & 0.234 & 0.088 \\
$\phi\times1$, mix$\times0.5$ & 0.005 & 0.139 & 0.049 \\
$\phi\times1$, mix$\times2$ & 0.019 & 0.343 & 0.147 \\
\hline\hline
\end{tabular}
\end{table}
...
Under the explicit gate file \texttt{output/chi\_open\_system/open\_system\_micro\_baseline\_candidate.csv}, this setup is marked as \texttt{GO\_baseline\_candidate} for the current thresholds, but we keep it outside baseline until the bath-side microscopic derivation is completed.
