...
$\chi_N^{(LR)}(D)$ localized channel & Stabilized localized-direct full-scan grid profile (\texttt{Dgrid60}) from 2D fine extraction + scan-level profile injection (Appendix~\ref{app:chi_loc}; D60 bias audit) & Yes & Promote to full per-$(D,\eta,N)$ localized direct projection (no profile injection) \\
$\Gamma_{N,\ell}$ channel normalization ($A_\ell$) & Action-derived $\tilde A_\ell(D)$ baseline with stabilized localized-direct \texttt{Dgrid60} profile support and legacy constant-$A_\ell$ comparator & Yes & Extend to fully localized channel-resolved direct extraction over $(D,\eta,N)$ \\
