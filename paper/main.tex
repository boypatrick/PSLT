...
$B_N$ visibility profile $B_N(D)$ from EFT-operator normalization & Baseline \texttt{eft\_operator\_norm}: overlap-extracted operator inputs $(g_{iN},\lambda_N)$ + finite one-loop + LL-RG-normalized kernel trace ratio & Yes & Promote from profile-evaluated normalization to per-cell direct operator extraction over full $(D,\eta,N)$ map \\
...
With the EFT-operator-normalized baseline now in place, the next step is to replace profile-evaluated normalization by full per-cell direct operator extraction over $(D,\eta,N)$ while extending the EYMH setup with an explicit lepton Yukawa sector in the same conformal background:
\begin{equation}
...
\end{equation}
Here $f_{L,R}$ denote lepton profile functions and $\mathcal{W}_{\rm frame}$ collects fixed normalization factors from the chosen conformal-frame convention. In the minimal Dirac-conformal baseline implied by Eqs.~\eqref{eq:dirac_higgs_weyl}--\eqref{eq:yukawa_frame_cancel}, $\mathcal{W}_{\rm frame}=1$ (implemented as \texttt{frame\_power=0} in \texttt{extract\_y\_eff\_2d\_three\_channel.py}). Nonzero \texttt{frame\_power} is kept only as a diagnostic sensitivity direction and is not used in baseline figures. In this roadmap, the same 2D localized solver and convergence criteria as Appendix~\ref{app:chi_loc} are reused; the current scan-level implementation already uses this operator-normalized visibility map, while full per-cell direct operator extraction remains future work.
...
To make \texttt{full\_direct} a release-level primary path (rather than an audit side branch), we define a reproducible \texttt{full\_direct\_map} publish mode in \texttt{code/publish\_full\_direct\_map.py}. This mode exports one integrated table from three evidence layers: (i) a full baseline map in strict \texttt{full\_direct} mode (\texttt{D60$\times$E60}), (ii) a small-surface complete localized-direct audit (\texttt{D21$\times$E41}), and (iii) a large-surface spot-check localized-direct audit (\texttt{D60$\times$E21}), with an additional \texttt{auto} vs \texttt{full\_direct} chain-mode parity row on \texttt{D21$\times$E41}. Results are summarized in Table~\ref{tab:full_direct_release} (source CSV: \texttt{paper/full\_direct\_map\_release\_summary.csv}).
...
chain\_mode\_parity\_auto\_vs\_full\_direct & D21$\times$E41 & 861 & 0.1243 & $2.11\times10^{-1}$ & --- & 0 \\
\hline\hline
\end{tabular}
\end{table*}
...
\begin{enumerate}
    ...
    \item \textbf{Model-chain unification}: $\chi_N^{(LR)}(D)$ and $\tilde A_\ell(D)$ are injected from action-derived full-grid profiles in the global $(D,\eta)$ map, while baseline $g_N$ uses \texttt{fp\_2d\_full} profile closure. The release baseline uses \texttt{--chain-mode full\_direct} as primary (strict D-grid lookup, no interpolation fallback), while \texttt{--chain-mode auto} is kept as comparator. A runtime-direct extension (\texttt{--chain-mode full\_direct\_runtime}) rebuilds active D-grid localized-direct profiles inside the scan entrypoint before strict evaluation, and an extreme no-profile-object mode (\texttt{--chain-mode cell\_direct\_runtime}) evaluates $g_N(D)$, $\chi_{LR}(D)$, and $\tilde A_\ell(D)$ by direct solvers inside map evaluation. On a medium grid parity audit (\texttt{Dgrid21$\times$Egrid41}), map-level drifts between \texttt{auto} and \texttt{full\_direct} are numerically zero in the recorded metrics (source CSV: \texttt{paper/chain\_mode\_full\_direct\_audit\_Dgrid21\_Egrid41.csv}); for the UV-to-EFT map at \texttt{Dgrid21$\times$Egrid21}, residual differences are at floating-point level (source CSV: \texttt{paper/chain\_mode\_uv\_audit\_Dgrid21\_Egrid21.csv}). The remaining gap is promotion of per-cell direct mode from stress-scale grids to release-scale production maps under realistic compute budgets.
