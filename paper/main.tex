...
\end{equation}
As in the 1D candidate check, this benchmark is reported as a replacement candidate. To test map-level impact, we connected both candidates to the main scan through \texttt{g\_mode} choices (\texttt{fp\_2d\_full}, \texttt{cardy}, \texttt{fp\_1d}, \texttt{fp\_2d}) and the reproducible script \texttt{scan\_gn\_profile\_impact.py}. Results in Table~\ref{tab:gn_profile_impact} show that the main conclusion is retained: relative to baseline \texttt{fp\_2d\_full}, the largest absolute shift is $0.0669$ in $f(\mathcal{R}_3>0.90)$ (for \texttt{fp\_2d}), while high-$N$ runaway remains controlled with $f(N_{\rm win}>3)=0$ in all tested modes.
...
open-system base $(1,1)$ & 0.097 & 0.160 & 0.124 \\
...
Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the generation metrics remain unchanged on this grid ($f(\mathcal{R}_3>0.90)=0.932$, $f(\mathcal{R}_3>0.95)=0.217$, $f(N_{\rm win}>3)=0$), while the $H\to\mu\mu$ acceptance fraction shifts within a controlled band $f(\chi^2_{\mu\mu}<4)\in[0.1500,\,0.1603]$ around the localized reference. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.
