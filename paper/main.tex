...
$g_N$ Cardy envelope + $q^{(N-1)^2}$ regulator & EFT baseline + fp\_2d impact-tested migration comparator & Yes & Promote phase-space counting profile as primary replacement if map-level stability is retained \\
...
To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map, same $\chi_{LR}(D)$ interpolation, and all other settings fixed). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, $f(\mathcal{R}_3>0.90)$ shifts by at most $0.10$ (from the $\nu$ sweep), while $f(\chi_{\mu\mu}^2<4)$ remains unchanged at $0.0942$ and $f(N_{\rm win}>3)=0$ in all tested cases.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.8333/0.8000/0.7833$ & $0.0942/0.0942/0.0942$ & $\Delta f_{\mathcal{R}_3}=0.0333,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.7000/0.8000/0.8667$ & $0.0942/0.0942/0.0942$ & $\Delta f_{\mathcal{R}_3}=0.1000,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.8333/0.8000/0.7667$ & $0.0942/0.0942/0.0942$ & $\Delta f_{\mathcal{R}_3}=0.0333,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.0942,
\end{equation}
with best grid point $\chi^2\simeq 3.0\times10^{-7}$ at $(D,\eta)\approx(9.97,1.36)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With D-interpolated $\chi_N^{(LR)}$, the $H\to\mu\mu$ illustrative proxy acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $9.4\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
At the same time, the present implementation keeps three EFT-level surrogates explicit: the Cardy-controlled $g_N$ envelope, the Yukawa-anchored visibility law $B_N$, and the $H\to\mu\mu$ proxy map in Eq.~\eqref{eq:mu_pred}.
...
\end{equation}
As in the 1D candidate check, this benchmark is reported as a replacement candidate. To test map-level impact, we connected both candidates to the main scan through \texttt{g\_mode} choices (\texttt{cardy}, \texttt{fp\_1d}, \texttt{fp\_2d}) and the reproducible script \texttt{scan\_gn\_profile\_impact.py}. Results in Table~\ref{tab:gn_profile_impact} show that the main conclusion is retained: $f(\mathcal{R}_3>0.90)$ shifts by at most $0.0167$ (absolute), while high-$N$ runaway remains controlled with $f(N_{\rm win}>3)=0$ in all tested modes.
...
\begin{figure*}[t]
    ...
    \caption{Low-$N$ comparison between Cardy baseline and 2D phase-space profile on $D=\{6,12,18\}$, shown as normalized ratios $g_N/g_3$ (log scale). The highlighted band marks $N=1,2,3$. Source files: \texttt{paper/gn\_cardy\_vs\_phase\_space.png}, \texttt{paper/gn\_cardy\_vs\_phase\_space.csv}.}
    \label{fig:gn_cardy_vs_phase_space}
\end{figure*}

\begin{table}[t]
\centering
\caption{Map-level impact of first-principles $g_N$ profiles on the $60\times60$ scan (diagnostic only; baseline remains Cardy mode). Source CSV: \texttt{paper/gn\_profile\_impact.csv}.}
...
\hline
baseline Cardy & 0.800 & 0.0942 & 0.000 & 0.0873 \\
...
\hline
baseline Cardy & 20 & 0.800 & 0.0942 & 0.000 & 0.087284 \\
baseline Cardy & 30 & 0.800 & 0.0942 & 0.000 & 0.087284 \\
baseline Cardy & 40 & 0.800 & 0.0942 & 0.000 & 0.087284 \\
...
\subsection{Baseline-Replacement Stress Test: \texorpdfstring{$g\_mode=\mathrm{fp\_2d\_full}$}{g_mode=fp_2d_full}}
To test a strict replacement scenario (not only low-$N$ shape correction), we added an upgraded full-tail mode \texttt{g\_mode=fp\_2d\_full}. Instead of direct low-$N$ substitution plus geometric continuation, this upgrade uses an explicit bounded microcanonical window for $N\le 3$ and an energy-gap tail prescription for $N>3$:
\begin{equation}
...
\end{equation}
followed by the same per-step finite-volume clipping used in \texttt{pslt\_lib.py}. In the present stress test, we use $(\alpha,\beta,s)=(0.8,1.1,0.0)$. Results are summarized in Table~\ref{tab:gn_baseline_replacement} (source CSV: \texttt{paper/gn\_baseline\_replacement.csv}). Relative to baseline Cardy, this upgraded full replacement keeps $f(\mathcal{R}_3>0.90)$ unchanged at $0.800$ ($\Delta=0.000$), with a small shift in $f(\chi^2_{\mu\mu}<4)$ to $0.0908$ and $f(N_{\rm win}>3)=0$ still controlled. Therefore, the map-level main conclusion is retained under this same-framework upgrade.

\begin{table}[t]
\centering
\caption{Map-level baseline-replacement stress test for $g_N$: Cardy baseline versus full 2D phase-space-tail mode (\texttt{fp\_2d\_full}) on the same $60\times60$ grid. Source CSV: \texttt{paper/gn\_baseline\_replacement.csv}.}
...
\hline
baseline Cardy & 0.800 & 0.0942 & 0.000 & 0.000 \\
$g_N$ fp\_2d full-tail replacement & 0.800 & 0.0908 & 0.000 & 0.000 \\
\hline\hline
\end{tabular}
\end{table}
...
Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the core scan summaries remain unchanged on this grid: $f(\mathcal{R}_3>0.90)=0.800$, $f(\mathcal{R}_3>0.95)=0$, $f(\chi^2_{\mu\mu}<4)=0.0942$, and $f(N_{\rm win}>3)=0$. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.

\subsection{Cross-Module Migration Snapshot}
To compare the two main first-principles migration directions on the same map-level metrics, we aggregate three scenarios: baseline (\texttt{cardy + localized}), $g_N$ profile replacement (\texttt{fp\_2d + localized}), and open-system $\chi$ (\texttt{cardy + open\_system}). The reproducible script is \texttt{scan\_first\_principles\_migration\_summary.py}; outputs are \texttt{paper/first\_principles\_migration\_summary.csv} and Fig.~\ref{fig:first_principles_migration}.
...
\hline
baseline (\texttt{cardy + localized}) & 0.800 & 0.0000 & 0.0942 & 0.0000 \\
$g_N$ migration (\texttt{fp\_2d + localized}) & 0.783 & $-0.0167$ & 0.0942 & 0.0000 \\
$\chi$ migration (\texttt{cardy + open\_system}) & 0.800 & 0.0000 & 0.0942 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
