...
The EFT/Wilson-matched mapping yields a restricted acceptance band (about $16.1\%$ of the scanned grid for $\chi^2<4$), providing a sharper falsifiable constraint on $(D,\eta)$.
\end{abstract}
...
To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map and full-grid action-derived $\chi_{LR}(D)$/$\tilde A_\ell(D)$ profiles). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, the key fractions remain unchanged: $f(\mathcal{R}_3>0.90)=0.9269$, $f(\chi_{\mu\mu}^2<4)=0.1614$, and $f(N_{\rm win}>3)\approx2.78\times10^{-4}$.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.9269/0.9269/0.9269$ & $0.1614/0.1614/0.1614$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.9269/0.9269/0.9269$ & $0.1614/0.1614/0.1614$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9269/0.9269/0.9269$ & $0.1614/0.1614/0.1614$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}

\subsection{H to mumu Signal Strength (EFT/Wilson-Matched Mapping)}
...
Within the PSLT closure, we construct a scan-level EFT/Wilson-matched map by combining kinetic occupancy with overlap-extracted raw couplings:
\begin{equation}
...
Equation~\eqref{eq:mu_pred} is implemented in \texttt{pslt\_lib.py} with \texttt{hll\_observable\_mode=eft\_wilson\_matched}; it is still a scan-level EFT module (not yet a full UV-to-EFT derivation from loop-level EYMH matching).
...
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.1614,
\end{equation}
with best grid point $\chi^2\simeq 6.39\times10^{-5}$ at $(D,\eta)\approx(8.07,2.71)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.48\linewidth]{hmumu_exclusion.png}
    \includegraphics[width=0.48\linewidth]{hmumu_signal_strength.png}
    \caption{Compatibility with $H\to\mu\mu$ under the EFT/Wilson-matched map. Left: $\chi^2$ map showing a restricted illustrative accepted band ($\chi^2<4$, equivalently $p_{\rm local}>4.55\times10^{-2}$ for dof$=1$). Right: predicted $\mu_{\mu\mu}^{\rm pred}$.}
    \label{fig:hmumu}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With full-grid action-derived $\chi_N^{(LR)}(D)$ and $\tilde A_\ell(D)$ profiles, the $H\to\mu\mu$ illustrative EFT/Wilson-matched acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $16.1\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
...
\hline
baseline fp\_2d\_full & 0.927 & 0.1614 & 0.0003 & 0.1267 \\
legacy Cardy reference & 0.877 & 0.1592 & 0.0003 & 0.1125 \\
$g_N$ fp\_1d profile & 0.860 & 0.1597 & 0.0003 & 0.1137 \\
$g_N$ fp\_2d profile & 0.860 & 0.1597 & 0.0003 & 0.1144 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.927 & 0.1614 & 0.0003 & 0.126709 \\
baseline fp\_2d\_full & 30 & 0.927 & 0.1614 & 0.0003 & 0.126709 \\
baseline fp\_2d\_full & 40 & 0.927 & 0.1614 & 0.0003 & 0.126709 \\
$g_N$ fp\_2d profile & 20 & 0.860 & 0.1597 & 0.0003 & 0.114352 \\
$g_N$ fp\_2d profile & 30 & 0.860 & 0.1597 & 0.0003 & 0.114352 \\
$g_N$ fp\_2d profile & 40 & 0.860 & 0.1597 & 0.0003 & 0.114352 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 0.9269 & 0.1614 & 0.0003 & 0.0000 \\
legacy Cardy reference & 0.8769 & 0.1592 & 0.0003 & -0.0500 \\
\hline\hline
\end{tabular}
\end{table}
...
Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the generation metrics remain unchanged on this grid ($f(\mathcal{R}_3>0.90)=0.932$, $f(\mathcal{R}_3>0.95)=0.217$, $f(N_{\rm win}>3)\simeq2.78\times10^{-4}$), while the $H\to\mu\mu$ acceptance fraction shifts within a controlled band $f(\chi^2_{\mu\mu}<4)\in[0.1500,\,0.1558]$ around the localized reference. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.
...
\hline
baseline (\texttt{fp\_2d\_full + localized\_grid}) & 0.927 & 0.0000 & 0.1614 & 0.0003 \\
legacy reference (\texttt{cardy + localized\_grid}) & 0.877 & -0.0500 & 0.1592 & 0.0003 \\
...
\hline
fixed-grid & $(9.97,\ 0.97)$ & $8.32\times10^{-1}$ & $0.1611$ & $3.06\times10^{-6}$ \\
$\chi^2$-best & $(8.07,\ 2.71)$ & $6.39\times10^{-5}$ & $0.1028$ & $7.47\times10^{-6}$ \\
robust-center & $(8.61,\ 3.29)$ & $8.55\times10^{-1}$ & $0.1189$ & $1.67\times10^{-5}$ \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{equation}
f(\chi^2_{\mu\mu}<4)\in[0.0231,\,0.2339],\qquad
\chi^2_{\min}\in[2.28\times10^{-9},\,4.36\times10^{-2}].
\end{equation}
The baseline fixed-anchor value used in main figures is $f(\chi^2_{\mu\mu}<4)=0.1614$.
