...
$H\to\mu\mu$ mapping [Eq.~\eqref{eq:mu_pred}] & EFT/Wilson-matched baseline + UV-tree/LL-RG audit map & Yes & Extend current UV-tree + LL-RG closure to full UV-to-EFT matching from EYMH action (operator basis + loop-level matching) \\
\bottomrule
\end{tabular}
\end{table*}
...
\end{equation}
\begin{equation}
C_{eH}^{ij}(D,\eta)=\sum_{N=1}^{3}Y_{iN}(D,\eta)\,P_N^{\rm(kin)}(D,\eta)\,Y_{jN}(D,\eta),
\label{eq:wilson_matched}
\end{equation}
\begin{equation}
...
To push this interface toward UV-to-EFT matching, we additionally run a reproducible UV-tree $\rightarrow$ LL-RG audit with \texttt{scan\_hll\_uv\_to\_eft\_matching.py}, exporting per-point $C_{eH}^{\mu\mu}(\mu_{\rm match})$, $C_{eH}^{\mu\mu}(\mu_{\rm low})$, and running metadata over the same $(D,\eta)$ grid (\texttt{paper/hll\_uv\_to\_eft\_map.csv}, \texttt{paper/hll\_uv\_to\_eft\_summary.csv}). In the baseline UV settings used here ($\texttt{uv\_blend}=0$, $\texttt{uv\_m2\_power}=1$), the map-level drift is bounded: $\langle |\Delta \mu_{\mu\mu}|\rangle \simeq 6.19\times10^{-4}$ with $\max |\Delta \mu_{\mu\mu}|\simeq 8.93\times10^{-3}$, while $f(\chi^2_{\mu\mu}<4)$ is unchanged between UV-tree and UV+LL-RG in this scan.
\begin{equation}
