...
(iii) a \textbf{Visibility module} ($B_N$) implemented from overlap-defined $y_N^{\rm eff}(D)$ profiles extracted by the same 2D localized operator chain, with branch tracking and microcanonical window averaging for mode-switch control, and high-$N$ saturation ($B_{N>3}=1$) to avoid double-counting with the $g_N$ regulator.
...
\begin{enumerate}
    ...
    \item \textbf{Overlap-Defined Visibility}: We promote $B_N$ to overlap-defined $y_N^{\rm eff}(D)$ profiles from the 2D localized solver, stabilized by branch tracking and microcanonical windows; a Yukawa law is retained only as a legacy comparator/fallback.
...
$B_N$ visibility profile $B_N(D)$ from overlap-defined $y_N^{\rm eff}$ & Action-derived profile extraction + tracked microcanonical windowing & Yes & Upgrade to fully normalized EFT operator matching (Section~\ref{sec:yukawa_roadmap}) \\
...
In early versions of the theory, $B_N$ was an arbitrary matching factor. In the present baseline, $B_N(D)$ is extracted from overlap-defined $y_N^{\rm eff}(D)$ profiles using the same 2D localized operator chain as Appendix~\ref{app:chi_loc}, with mode-tracking and microcanonical-window averaging to suppress branch-switch artifacts; high-$N$ regularization remains isolated in Eq.~\eqref{eq:gN_reg}.
...
Beyond the current overlap-profile baseline, the next step is a full EFT-level normalization/matching derivation by extending the EYMH setup with an explicit lepton Yukawa sector in the same conformal background:
\begin{equation}
...
\end{equation}
Here $f_{L,R}$ denote lepton profile functions and $\mathcal{W}_{\rm frame}$ collects fixed normalization factors from the chosen conformal-frame convention. In the minimal Dirac-conformal baseline implied by Eqs.~\eqref{eq:dirac_higgs_weyl}--\eqref{eq:yukawa_frame_cancel}, $\mathcal{W}_{\rm frame}=1$ (implemented as \texttt{frame\_power=0} in \texttt{extract\_y\_eff\_2d\_three\_channel.py}). Nonzero \texttt{frame\_power} is kept only as a diagnostic sensitivity direction and is not used in baseline figures. In this roadmap, the same 2D localized solver and convergence criteria as Appendix~\ref{app:chi_loc} are reused; the current scan-level implementation already uses this overlap profile at map level, while full operator normalization/matching remains future work.
...
\midrule
Visibility $B_N$ & \texttt{b\_mode} & \texttt{overlap\_2d} & Baseline profile from overlap-defined $y_N^{\rm eff}(D)$ (tracked + microcanonical-windowed) \\
& $p_B$ & 0.30 & Legacy Yukawa comparator/fallback parameter (inactive in overlap baseline) \\
\midrule
...
\begin{enumerate}
    ...
    \item \textbf{Stability without ad-hoc cutoffs}: The infinite tower of layers becomes numerically and physically stable once micro-degeneracy is regulated by the \texttt{fp\_2d\_full} microcanonical-window + tail prescription and the observable sector uses overlap-defined visibility profiles $B_N(D)$ with explicit high-$N$ saturation.
...
The use of mixed-resolution ingredients is a deliberate design choice: action-derived extraction fixes low-mode profiles (\texttt{fp\_2d\_full} for $g_N$, overlap $B_N(D)$, $\chi_N^{(LR)}(D)$, and $\tilde A_\ell(D)$), while global-map evaluation is still an EFT-level closure rather than a full direct localized recomputation at every $(D,\eta,N)$ point.
\begin{enumerate}
...
To demonstrate that the three-generation structure is robust and not an artifact of a particular ansatz choice, we compare the baseline overlap-derived $B_N(D)$ profile (this work) against the alternative ``inverse Yukawa'' ansatz explored in earlier versions.
...
\begin{itemize}
    \item \textbf{Baseline (overlap-derived profile)}: $B_1(D) < B_2(D) < B_3$ in the scanned range, with smooth $D$-dependence from tracked microcanonical overlaps. The three-generation structure emerges from the $g_N$--$\Gamma_N$ competition modulated by these profile differences.
