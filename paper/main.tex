...
\midrule
$\Omega(\rho,z;D)$ two-center geometry & Model ansatz, action-compatible & Yes & Derive from a specific higher-dimensional parent solution \\
...
We consider a two-center conformal factor with harmonic structure:
\begin{equation}
\boxed{
...
\end{equation}
where $(\rho, z)$ are cylindrical coordinates (axisymmetric, $m=0$), $x_\pm = \pm (D/2) \hat{z}$ are the two center positions, $a$ is the conformal factor strength, and $\varepsilon$ is a core regularization parameter that smears the $\delta$-function sources:
\begin{equation}
...
\end{equation}
This form is directly analogous to superposed harmonic functions in gravitational potential theory.
...
\begin{enumerate}
    ...
    \item \textbf{Finite-time dynamics}: $t_{\rm coh}$ is treated as a control parameter in this version; a fully geometry-closed $t_{\rm coh}(D,\eta,N)$ prescription is still under development.
\end{enumerate}
...
\subsection{Minimal Lindblad Demonstrator for Open-System \texorpdfstring{$\chi$}{chi}}
As a methodological bridge (not a replacement of Appendix~\ref{app:chi_loc}), we provide a minimal two-level open-system model:
\begin{equation}
...
\end{equation}
The script \texttt{code/lindblad\_chi\_minimal.py} outputs reproducible diagnostics such as
\begin{equation}
C_{\max}\equiv \max_t |\rho_{LR}(t)|,\qquad P_{\max}\equiv \max_t \rho_{RR}(t),
\end{equation}
and an optional normalized proxy $\chi_{\rm eff}^{\rm(proxy)}=2\gamma_{\rm mix}C_{\max}/\bar{\Gamma}_N$. For the default demonstrator point ($\Delta=0.358108$, $\gamma_\phi=0.05$, $\gamma_{\rm mix}=0.01$, $\bar{\Gamma}_N=808.7$), we obtain
\begin{equation}
C_{\max}=3.775\times10^{-1},\qquad P_{\max}=7.693\times10^{-1},\qquad \chi_{\rm eff}^{\rm(proxy)}=9.34\times10^{-6}.
\end{equation}
These numbers are model-dependent because $(\gamma_\phi,\gamma_{\rm mix})$ are not yet derived from a microscopic EYMH bath model; therefore this open-system block remains outside all reported scan statistics.
