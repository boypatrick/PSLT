...
We verify the closure against two benchmarks: (1) internal stability of the three-generation hierarchy (Generation Ratio $> 90\%$ over $98.3\%$ of the sampled $(D,\eta)$ grid), and (2) a proxy compatibility test with the ATLAS Run-3 $H\to\mu\mu$ signal strength ($\mu_{\mu\mu}^{\rm obs}=1.4\pm0.4$).
The proxy mapping yields a restricted acceptance band (about $9.6\%$ of the scanned grid for $\chi^2<4$), providing a sharper falsifiable constraint on $(D,\eta)$.
\end{abstract}
...
\begin{figure}[htbp]
    ...
    \caption{One-page action-derived chain overview used in this manuscript: geometry input $\rightarrow$ operator extraction $\rightarrow$ scan diagnostics $\rightarrow$ observable proxy checks. This diagram is aligned with the repository reproducibility entrypoint and Appendix~\ref{app:repro_sop}.}
...
To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map, same $\chi_{LR}(D)$ interpolation, and all other settings fixed). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, the key fractions remain unchanged: $f(\mathcal{R}_3>0.90)=0.9833$, $f(\chi_{\mu\mu}^2<4)=0.0961$, and $f(N_{\rm win}>3)=0$.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.9833/0.9833/0.9833$ & $0.0961/0.0961/0.0961$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.9833/0.9833/0.9833$ & $0.0961/0.0961/0.0961$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9833/0.9833/0.9833$ & $0.0961/0.0961/0.0961$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}
...
\label{eq:mu_pred}
\end{equation}
This is a \emph{proxy mapping}, not a first-principles EFT derivation. We assume that the effective $h\mu\mu$ coupling scales with layer weight: $g_{h\mu\mu}^{\rm eff} \propto \sqrt{W_2}$. By construction, Eq.~\eqref{eq:mu_pred} is a map-level normalization proxy and should not be interpreted as a Wilson-coefficient extraction. Deriving this mapping from the EYMH action remains an open problem.
...
\end{equation}
Figure~\ref{fig:hmumu} shows the \textbf{proxy acceptance region} in two equivalent languages,
\begin{equation}
\chi^2<4 \quad \Longleftrightarrow \quad p_{\rm local}>4.55\times10^{-2}\qquad(\text{dof}=1).
\label{eq:chi2_plocal_equiv}
\end{equation}
We use this threshold only as an \emph{illustrative acceptance band} for map-level comparison and do not claim a formal 95\% CL exclusion. Because the $(D,\eta)$ scan is a multiple-comparison procedure (look-elsewhere) and no global trials correction is applied, this band is interpreted only as a falsifiable target region. In the present D-interpolated $\chi_N^{(LR)}$ scan, the proxy-accepted fraction is
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.0961,
\end{equation}
with best grid point $\chi^2\simeq 7.28\times10^{-8}$ at $(D,\eta)\approx(11.05,3.16)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.48\linewidth]{hmumu_exclusion.png}
    \includegraphics[width=0.48\linewidth]{hmumu_signal_strength.png}
    \caption{Compatibility with $H\to\mu\mu$. Left: $\chi^2$ map showing a restricted illustrative proxy-accepted band ($\chi^2<4$, equivalently $p_{\rm local}>4.55\times10^{-2}$ for dof$=1$). Right: Predicted signal strength proxy.}
    \label{fig:hmumu}
\end{figure}

\subsection{Proxy Extension to H to ee and H to tautau}
Using the same map-level proxy definition, we additionally evaluate lepton-channel signal strengths by layer assignment $N=1\rightarrow ee$, $N=2\rightarrow \mu\mu$, and $N=3\rightarrow \tau\tau$:
\begin{equation}
\mu_{\ell\ell}^{\rm pred}(D,\eta)=\frac{W_N(D,\eta)}{W_N(D_0,\eta_0)},\qquad
W_N=B_N\,g_N\left(1-\ee^{-\Gamma_N t_{\rm coh}}\right).
\end{equation}
Figure~\ref{fig:hll_proxy} summarizes the corresponding proxy maps on the same $(D,\eta)$ grid. The tabulated summary statistics are provided in \texttt{paper/hll\_signal\_strength\_summary.csv}.
This extension remains a map-level proxy exercise for the observable mapping itself, but the visibility sector in the global scan is now implemented with explicit overlap-defined $y_N^{\rm eff}(D)$ profiles (extracted by \texttt{extract\_y\_eff\_2d\_three\_channel.py} with mode tracking and microcanonical windowing). The repository keeps three-channel observation fields (\texttt{H\_to\_ee}, \texttt{H\_to\_mumu}, \texttt{H\_to\_tautau}) in \texttt{data/pdg\_leptons.json} as a reproducible interface.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\linewidth]{hll_signal_strength_maps.png}
    \caption{PSLT proxy extension from $H\to\mu\mu$ to three lepton channels on the same scan grid: $H\to ee$ (left), $H\to\mu\mu$ (middle), and $H\to\tau\tau$ (right). Each panel shows $\mu_{\ell\ell}^{\rm pred}=W_N/W_N^{\rm ref}$ with the same reference point $(D_0,\eta_0)=(10,1)$.}
    \label{fig:hll_proxy}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With D-interpolated $\chi_N^{(LR)}$, the $H\to\mu\mu$ illustrative proxy acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $9.6\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
At the same time, the present implementation keeps two EFT-level surrogates explicit: the Cardy-normalized \texttt{fp\_2d\_full} $g_N$ closure and the $H\to\mu\mu$ proxy map in Eq.~\eqref{eq:mu_pred}.
...
\begin{enumerate}
    \item \textbf{Observable mapping}: the $H\to\mu\mu$ comparison uses the proxy $g_{h\mu\mu}^{\rm eff}\propto \sqrt{W_2}$ rather than a full derivation from the EYMH action. The scan now uses overlap-defined visibility profiles from Eq.~\eqref{eq:yukawa_eff_roadmap}-type overlaps, but EFT-level vertex matching is still pending.
...
\hline
Global maps and proxy plots & \texttt{generate\_plots.py}; \texttt{scan\_hll\_signal\_strengths.py} & \texttt{output/three\_generation\_*.png}; \texttt{output/hmumu\_*.png}; \texttt{output/hll\_signal\_strength/*} \\
...
\hline
Linear interpolation & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
Exponential fit      & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
Linear $\times 0.8$  & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
Linear $\times 1.2$  & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
Exp-fit $\times 0.8$ & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
Exp-fit $\times 1.2$ & $0.983$ & $0.217$ & $0.096$ & $(11.05,\;3.16)$ \\
\hline\hline
\end{tabular}
\end{table*}
...
\hline
baseline fp\_2d\_full & 0.983 & 0.0961 & 0.000 & 0.0596 \\
legacy Cardy reference & 0.933 & 0.0972 & 0.000 & 0.0424 \\
$g_N$ fp\_1d profile & 0.933 & 0.0972 & 0.000 & 0.0437 \\
$g_N$ fp\_2d profile & 0.933 & 0.0972 & 0.000 & 0.0444 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.983 & 0.0961 & 0.000 & 0.059601 \\
baseline fp\_2d\_full & 30 & 0.983 & 0.0961 & 0.000 & 0.059601 \\
baseline fp\_2d\_full & 40 & 0.983 & 0.0961 & 0.000 & 0.059601 \\
$g_N$ fp\_2d profile & 20 & 0.933 & 0.0972 & 0.000 & 0.044400 \\
$g_N$ fp\_2d profile & 30 & 0.933 & 0.0972 & 0.000 & 0.044400 \\
$g_N$ fp\_2d profile & 40 & 0.933 & 0.0972 & 0.000 & 0.044400 \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
followed by the same per-step finite-volume clipping used in \texttt{pslt\_lib.py}. In the present baseline run, we use $(\alpha,\beta,s)=(0.8,1.1,0.0)$. Results are summarized in Table~\ref{tab:gn_baseline_replacement} (source CSV: \texttt{paper/gn\_baseline\_replacement.csv}). Relative to baseline \texttt{fp\_2d\_full}, the legacy Cardy reference lowers $f(\mathcal{R}_3>0.90)$ by $0.05$ (from $0.983$ to $0.933$), shifts $f(\chi^2_{\mu\mu}<4)$ by only $+0.0011$, and still yields $f(N_{\rm win}>3)=0$. Therefore, the map-level main conclusion is retained under this baseline switch.
...
\hline
baseline fp\_2d\_full & 0.983 & 0.0961 & 0.000 & 0.000 \\
legacy Cardy reference & 0.933 & 0.0972 & 0.000 & -0.050 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline (\texttt{fp\_2d\_full + localized}) & 0.983 & 0.0000 & 0.0961 & 0.0000 \\
legacy reference (\texttt{cardy + localized}) & 0.933 & -0.0500 & 0.0972 & 0.0000 \\
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.983 & 0.0000 & 0.0961 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
constant $t_{\rm coh}=1$ & 0.983 & 0.217 & 0.0961 & $7.28\times10^{-8}$ \\
$\pi/\Delta\omega_{12}(D)$ & 0.983 & 0.217 & 0.3883 & $5.59\times10^{-9}$ \\
$\min(\pi/\Delta\omega_{12},10^4)$ & 0.983 & 0.217 & 0.1711 & $5.59\times10^{-9}$ \\
\hline\hline
\end{tabular}
\end{table}
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $\eta_{\rm eff}=\eta$ & 0.983 & 0.217 & 0.0961 & $7.28\times10^{-8}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm amp}(D)$ & 0.983 & 0.217 & 0.0956 & $1.29\times10^{-6}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm prob}(D)$ & 0.983 & 0.217 & 0.0947 & $4.94\times10^{-7}$ \\
$\eta_{\rm eff}=\eta_{\rm amp}(D)$ & 0.983 & 0.217 & 0.1000 & $5.87\times10^{-2}$ \\
$\eta_{\rm eff}=\eta_{\rm prob}(D)$ & 0.983 & 0.217 & 0.1000 & $5.69\times10^{-2}$ \\
\hline\hline
\end{tabular}
\end{table}
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $(A_1,A_2)=(1,1)$ & 0.9833 & 0.217 & 0.0961 & $7.28\times10^{-8}$ \\
$(A_1,A_2)=(\tilde A_1,\tilde A_2)$ & 0.9322 & 0.217 & 0.0367 & $3.67\times10^{-5}$ \\
$(A_1,A_2)=0.5(\tilde A_1,\tilde A_2)$ & 0.9108 & 0.217 & 0.0367 & $3.67\times10^{-5}$ \\
$(A_1,A_2)=2(\tilde A_1,\tilde A_2)$ & 0.9553 & 0.217 & 0.0367 & $3.67\times10^{-5}$ \\
\hline\hline
\end{tabular}
\end{table}
