...
The proxy mapping yields a restricted acceptance band (about $9.1\%$ of the scanned grid for $\chi^2<4$), providing a sharper falsifiable constraint on $(D,\eta)$.
\end{abstract}
...
\hline
$(4.0,0.2)$ & $3/3$ & $4.02\times10^{-4}$ & $5.27\times10^{-4}$ & $3.44\times10^{-11}$ & $2.80\times10^{-7}$ & yes \\
$(5.0,2.0)$ & $2/2$ & $4.02\times10^{-4}$ & $3.31\times10^{-4}$ & $4.93\times10^{-12}$ & $0$ & yes \\
$(11.0,1.4)$ & $1/1$ & $2.51\times10^{-4}$ & $3.06\times10^{-4}$ & $0$ & $0$ & yes \\
\hline\hline
\end{tabular}
\end{table}

To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map, same $\chi_{LR}(D)$ interpolation, and all other settings fixed). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, $f(\mathcal{R}_3>0.90)$ shifts by at most $0.1333$ (from the $p_B$ sweep), while $f(\chi_{\mu\mu}^2<4)$ remains unchanged at $0.0908$ and $f(N_{\rm win}>3)=0$ in all tested cases.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.8000/0.8000/0.8000$ & $0.0908/0.0908/0.0908$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.8000/0.8000/0.8000$ & $0.0908/0.0908/0.0908$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9333/0.8000/0.7333$ & $0.0908/0.0908/0.0908$ & $\Delta f_{\mathcal{R}_3}=0.1333,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.0908,
\end{equation}
with best grid point $\chi^2\simeq 2.0\times10^{-5}$ at $(D,\eta)\approx(9.97,1.36)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.
...
This extension remains a map-level proxy exercise: it does not yet replace the visibility sector with explicit overlap-defined $y_N^{\rm eff}$ operators in the global scan. The repository now includes three-channel observation fields (\texttt{H\_to\_ee}, \texttt{H\_to\_mumu}, \texttt{H\_to\_tautau}) in \texttt{data/pdg\_leptons.json} as a reproducible interface for the future overlap-operator migration.
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With D-interpolated $\chi_N^{(LR)}$, the $H\to\mu\mu$ illustrative proxy acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $9.1\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
At the same time, the present implementation keeps three EFT-level surrogates explicit: the Cardy-normalized \texttt{fp\_2d\_full} $g_N$ closure, the Yukawa-anchored visibility law $B_N$, and the $H\to\mu\mu$ proxy map in Eq.~\eqref{eq:mu_pred}.
...
\hline
baseline fp\_2d\_full & 0.800 & 0.0908 & 0.000 & 0.0864 \\
legacy Cardy reference & 0.800 & 0.0942 & 0.000 & 0.0873 \\
$g_N$ fp\_1d profile & 0.800 & 0.0942 & 0.000 & 0.0894 \\
$g_N$ fp\_2d profile & 0.783 & 0.0942 & 0.000 & 0.0905 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.800 & 0.0908 & 0.000 & 0.086434 \\
baseline fp\_2d\_full & 30 & 0.800 & 0.0908 & 0.000 & 0.086434 \\
baseline fp\_2d\_full & 40 & 0.800 & 0.0908 & 0.000 & 0.086434 \\
$g_N$ fp\_2d profile & 20 & 0.783 & 0.0942 & 0.000 & 0.090492 \\
$g_N$ fp\_2d profile & 30 & 0.783 & 0.0942 & 0.000 & 0.090492 \\
$g_N$ fp\_2d profile & 40 & 0.783 & 0.0942 & 0.000 & 0.090492 \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
followed by the same per-step finite-volume clipping used in \texttt{pslt\_lib.py}. In the present baseline run, we use $(\alpha,\beta,s)=(0.8,1.1,0.0)$. Results are summarized in Table~\ref{tab:gn_baseline_replacement} (source CSV: \texttt{paper/gn\_baseline\_replacement.csv}). Relative to baseline \texttt{fp\_2d\_full}, the legacy Cardy reference keeps $f(\mathcal{R}_3>0.90)$ unchanged at $0.800$ ($\Delta=0.000$), shifts $f(\chi^2_{\mu\mu}<4)$ by only $+0.0033$, and still yields $f(N_{\rm win}>3)=0$. Therefore, the map-level main conclusion is retained under this baseline switch.
...
\hline
baseline fp\_2d\_full & 0.800 & 0.0908 & 0.000 & 0.000 \\
legacy Cardy reference & 0.800 & 0.0942 & 0.000 & 0.000 \\
\hline\hline
\end{tabular}
\end{table}
...
Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the core scan summaries remain unchanged on this grid: $f(\mathcal{R}_3>0.90)=0.800$, $f(\mathcal{R}_3>0.95)=0$, $f(\chi^2_{\mu\mu}<4)=0.0908$, and $f(N_{\rm win}>3)=0$. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.
...
\hline
baseline (\texttt{fp\_2d\_full + localized}) & 0.800 & 0.0000 & 0.0908 & 0.0000 \\
legacy reference (\texttt{cardy + localized}) & 0.800 & 0.0000 & 0.0942 & 0.0000 \\
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.800 & 0.0000 & 0.0908 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
constant $t_{\rm coh}=1$ & 0.800 & 0.000 & 0.0908 & $2.03\times10^{-5}$ \\
$\pi/\Delta\omega_{12}(D)$ & 0.800 & 0.000 & 0.3681 & $3.21\times10^{-8}$ \\
$\min(\pi/\Delta\omega_{12},10^4)$ & 0.800 & 0.000 & 0.1825 & $1.29\times10^{-5}$ \\
\hline\hline
\end{tabular}
\end{table}
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $\eta_{\rm eff}=\eta$ & 0.800 & 0.000 & 0.0908 & $2.03\times10^{-5}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm amp}(D)$ & 0.800 & 0.000 & 0.0900 & $2.43\times10^{-5}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm prob}(D)$ & 0.800 & 0.000 & 0.0900 & $2.90\times10^{-5}$ \\
$\eta_{\rm eff}=\eta_{\rm amp}(D)$ & 0.800 & 0.000 & 0.0833 & $3.63\times10^{-2}$ \\
$\eta_{\rm eff}=\eta_{\rm prob}(D)$ & 0.800 & 0.000 & 0.0833 & $3.50\times10^{-2}$ \\
\hline\hline
\end{tabular}
\end{table}
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $(A_1,A_2)=(1,1)$ & 0.8000 & 0.000 & 0.0908 & $2.03\times10^{-5}$ \\
$(A_1,A_2)=(\tilde A_1,\tilde A_2)$ & 0.7961 & 0.000 & 0.0350 & $3.62\times10^{-5}$ \\
$(A_1,A_2)=0.5(\tilde A_1,\tilde A_2)$ & 0.7939 & 0.000 & 0.0350 & $3.62\times10^{-5}$ \\
$(A_1,A_2)=2(\tilde A_1,\tilde A_2)$ & 0.7981 & 0.000 & 0.0350 & $3.62\times10^{-5}$ \\
\hline\hline
\end{tabular}
\end{table}
