...
We verify the closure against two benchmarks: (1) internal stability of the three-generation hierarchy (Generation Ratio $> 90\%$ over $93.2\%$ of the sampled $(D,\eta)$ grid), and (2) a scan-level EFT/Wilson-matched compatibility test with the ATLAS Run-3 $H\to\mu\mu$ signal strength ($\mu_{\mu\mu}^{\rm obs}=1.4\pm0.4$).
The EFT/Wilson-matched mapping yields a restricted acceptance band (about $16.8\%$ of the scanned grid for $\chi^2<4$), providing a sharper falsifiable constraint on $(D,\eta)$.
\end{abstract}
...
$g_N$ phase-space full-tail mode (\texttt{fp\_2d\_full}) with phase-space normalization & Baseline mode + legacy Cardy reference retained for comparator audits & Yes & Extend from profile-interpolated $D$ support to direct full-grid action extraction \\
...
$\Gamma_{N,\ell}$ channel normalization ($A_\ell$) & Action-derived profile baseline $\tilde A_\ell(D)$ with constant-$A_\ell$ legacy comparator & Yes & Extend to full localized channel extraction without profile interpolation \\
...
\label{eq:R3}
\end{equation}
With $\chi_N^{(LR)}(D)$ interpolation from Appendix~\ref{app:chi_loc}, our numerical scan gives $\mathcal{R}_3 > 90\%$ over $93.2\%$ of the sampled $(D,\eta)$ grid, while $f(\mathcal{R}_3>0.95)=0.2167$ in this setup. The interpolation profile is anchored by the benchmark convergence set (Table~\ref{tab:chi_loc}) and the auxiliary full-scan support points (Table~\ref{tab:chi_loc_full}) in Appendix~\ref{app:chi_loc}. This still supports a dominant three-layer sector; the current baseline combines action-derived $g_N$ and $\tilde A_\ell(D)$ profiles with overlap-defined $B_N(D)$, while keeping profile interpolation in $\chi_N^{(LR)}(D)$ for map-level tractability.
...
To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map, same $\chi_{LR}(D)$ interpolation, and all other settings fixed). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, the key fractions remain unchanged: $f(\mathcal{R}_3>0.90)=0.9322$, $f(\chi_{\mu\mu}^2<4)=0.1681$, and $f(N_{\rm win}>3)=0$.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.9322/0.9322/0.9322$ & $0.1681/0.1681/0.1681$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.9322/0.9322/0.9322$ & $0.1681/0.1681/0.1681$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9322/0.9322/0.9322$ & $0.1681/0.1681/0.1681$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
We use this threshold only as an \emph{illustrative acceptance band} for map-level comparison and do not claim a formal 95\% CL exclusion. Because the $(D,\eta)$ scan is a multiple-comparison procedure (look-elsewhere) and no global trials correction is applied, this band is interpreted only as a falsifiable target region. In the present D-interpolated $\chi_N^{(LR)}$ scan, the accepted fraction is
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.1681,
\end{equation}
with best grid point $\chi^2\simeq 1.61\times10^{-4}$ at $(D,\eta)\approx(9.42,1.29)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With D-interpolated $\chi_N^{(LR)}$, the $H\to\mu\mu$ illustrative EFT/Wilson-matched acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $16.8\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
At the same time, the present implementation keeps two EFT-level approximations explicit: profile interpolation in the scan-level $g_N(D)$/$\chi_N^{(LR)}(D)$ transfer chain, and a bounded nearest-neighbor flavor projector in the matched $H\to\mu\mu$ map of Eq.~\eqref{eq:mu_pred}.
...
The use of mixed-resolution ingredients is a deliberate design choice: action-derived extraction fixes low-mode profiles (\texttt{fp\_2d\_full} for $g_N$, overlap $B_N(D)$, and $\tilde A_\ell(D)$), while the global $(D,\eta)$ map still relies on profile interpolation (especially for $\chi_N^{(LR)}(D)$) for tractable scans.
\begin{enumerate}
    ...
    \item \textbf{Model-chain unification}: $\chi_N^{(LR)}(D)$ is extracted from the action-derived 2D localized solver (Appendix~\ref{app:chi_loc}) and then profile-interpolated in the global $(D,\eta)$ map. A full end-to-end scan with direct localized recomputation at every grid point remains future work.
...
\hline
baseline fp\_2d\_full & 0.932 & 0.1681 & 0.000 & 0.1214 \\
legacy Cardy reference & 0.882 & 0.1647 & 0.000 & 0.1070 \\
$g_N$ fp\_1d profile & 0.867 & 0.1650 & 0.000 & 0.1082 \\
$g_N$ fp\_2d profile & 0.865 & 0.1650 & 0.000 & 0.1088 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.932 & 0.1681 & 0.000 & 0.121385 \\
baseline fp\_2d\_full & 30 & 0.932 & 0.1681 & 0.000 & 0.121385 \\
baseline fp\_2d\_full & 40 & 0.932 & 0.1681 & 0.000 & 0.121385 \\
$g_N$ fp\_2d profile & 20 & 0.865 & 0.1650 & 0.000 & 0.108846 \\
$g_N$ fp\_2d profile & 30 & 0.865 & 0.1650 & 0.000 & 0.108846 \\
$g_N$ fp\_2d profile & 40 & 0.865 & 0.1650 & 0.000 & 0.108846 \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
followed by the same per-step finite-volume clipping used in \texttt{pslt\_lib.py}. In the present baseline run, we use $(\alpha,\beta,s)=(0.8,1.1,0.0)$. Results are summarized in Table~\ref{tab:gn_baseline_replacement} (source CSV: \texttt{paper/gn\_baseline\_replacement.csv}). Relative to baseline \texttt{fp\_2d\_full}, the legacy Cardy reference lowers $f(\mathcal{R}_3>0.90)$ by $0.0503$ (from $0.932$ to $0.882$), lowers $f(\chi^2_{\mu\mu}<4)$ by $0.0033$ (from $0.1681$ to $0.1647$), and still yields $f(N_{\rm win}>3)=0$. Therefore, the map-level main conclusion is retained under this baseline switch.
...
\hline
baseline fp\_2d\_full & 0.932 & 0.1681 & 0.000 & 0.000 \\
legacy Cardy reference & 0.882 & 0.1647 & 0.000 & -0.0503 \\
\hline\hline
\end{tabular}
\end{table}
...
To compare the two main migration directions on the same map-level metrics under the updated baseline, we aggregate three scenarios: baseline (\texttt{fp\_2d\_full + localized}), legacy reference (\texttt{cardy + localized}), and open-system $\chi$ (\texttt{fp\_2d\_full + open\_system}). The reproducible script is \texttt{scan\_first\_principles\_migration\_summary.py}; outputs are \texttt{paper/first\_principles\_migration\_summary.csv} and Fig.~\ref{fig:first_principles_migration}.
...
\hline
baseline (\texttt{fp\_2d\_full + localized}) & 0.932 & 0.0000 & 0.1681 & 0.0000 \\
legacy reference (\texttt{cardy + localized}) & 0.882 & -0.0503 & 0.1647 & 0.0000 \\
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.932 & 0.0000 & 0.1500 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
fixed-grid & $(9.97,\ 0.97)$ & $8.39\times10^{-1}$ & $0.1664$ & $1.43\times10^{-6}$ \\
$\chi^2$-best & $(9.42,\ 1.29)$ & $1.61\times10^{-4}$ & $0.1475$ & $2.42\times10^{-8}$ \\
robust-center & $(8.61,\ 2.84)$ & $6.53\times10^{-2}$ & $0.1219$ & $1.35\times10^{-5}$ \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{equation}
f(\chi^2_{\mu\mu}<4)\in[0.0208,\,0.2414],\qquad
\chi^2_{\min}\in[3.71\times10^{-8},\,9.93\times10^{-3}].
\end{equation}
The baseline fixed-anchor value used in main figures is $f(\chi^2_{\mu\mu}<4)=0.1681$.
...
action-profile $(A_1,A_2)=(\tilde A_1,\tilde A_2)$ & 0.9322 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$(A_1,A_2)=0.5(\tilde A_1,\tilde A_2)$ & 0.9108 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$(A_1,A_2)=2(\tilde A_1,\tilde A_2)$ & 0.9553 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
\hline\hline
\end{tabular}
\end{table}
