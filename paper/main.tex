...
Open-system $\chi_{\rm eff}$ (Lindblad) & Scan-ready diagnostic module with profiled $\gamma_\phi(D),\gamma_{\rm mix}(D)$ (Appendix~\ref{app:chi_open_scan}) & Diagnostic only (not baseline) & Derive microscopic EYMH bath $(L_k,\gamma_k)$ and then promote to baseline if robust \\
...
\subsection{Geometry-Informed Lindblad Scan-Ready Module for Open-System \texorpdfstring{$\chi$}{chi}}
...
\begin{table}[t]
\centering
\caption{Open-system sensitivity scan with profiled $(\gamma_\phi,\gamma_{\rm mix})$ on the $60\times60$ map. Source CSV: \texttt{paper/chi\_open\_system\_sensitivity.csv}.}
...
open-system base $(1,1)$ & 0.085 & 0.213 & 0.127 \\
$\phi\times0.5$, mix$\times1$ & 0.146 & 0.216 & 0.178 \\
$\phi\times2$, mix$\times1$ & 0.059 & 0.115 & 0.082 \\
$\phi\times1$, mix$\times0.5$ & 0.058 & 0.109 & 0.079 \\
$\phi\times1$, mix$\times2$ & 0.148 & 0.228 & 0.184 \\
$\phi\times0.5$, mix$\times2$ & 0.206 & 0.275 & 0.232 \\
$\phi\times2$, mix$\times0.5$ & 0.032 & 0.068 & 0.046 \\
\hline\hline
\end{tabular}
\end{table}

Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the generation metrics remain unchanged on this grid ($f(\mathcal{R}_3>0.90)=0.932$, $f(\mathcal{R}_3>0.95)=0.217$, $f(N_{\rm win}>3)\simeq2.78\times10^{-4}$), while the $H\to\mu\mu$ acceptance fraction remains fixed at $f(\chi^2_{\mu\mu}<4)=0.1500$ in this setup. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.

\subsection{Cross-Module Migration Snapshot}
To compare the two main migration directions on the same map-level metrics under the updated baseline, we aggregate three scenarios: baseline (\texttt{fp\_2d\_full + localized\_grid}), legacy reference (\texttt{cardy + localized\_grid}), and open-system $\chi$ (\texttt{fp\_2d\_full + open\_system}). The reproducible script is \texttt{scan\_first\_principles\_migration\_summary.py}; outputs are \texttt{paper/first\_principles\_migration\_summary.csv} and Fig.~\ref{fig:first_principles_migration}.
...
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.927 & 0.0000 & 0.1500 & 0.0003 \\
\hline\hline
\end{tabular}
\end{table}
