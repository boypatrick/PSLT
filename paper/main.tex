...
$H\to\mu\mu$ mapping [Eq.~\eqref{eq:mu_pred}] & EFT/Wilson-matched map (scan-level) & Yes & Extend to full UV-to-EFT matching from EYMH action (operator basis + loop-level matching) \\
\bottomrule
\end{tabular}
\end{table*}
...
\hline
Global maps and EFT/Wilson-matched observable plots & \texttt{generate\_plots.py}; \texttt{scan\_hll\_signal\_strengths.py} & \texttt{output/three\_generation\_*.png}; \texttt{output/hmumu\_*.png}; \texttt{output/hll\_signal\_strength/*} \\
