...
We verify the closure against two benchmarks: (1) internal stability of the three-generation hierarchy (Generation Ratio $> 90\%$ over $98.3\%$ of the sampled $(D,\eta)$ grid), and (2) an EFT-diagonal Wilson-coefficient compatibility test with the ATLAS Run-3 $H\to\mu\mu$ signal strength ($\mu_{\mu\mu}^{\rm obs}=1.4\pm0.4$).
...
\label{eq:R3}
\end{equation}
With $\chi_N^{(LR)}(D)$ interpolation from Appendix~\ref{app:chi_loc}, our numerical scan gives $\mathcal{R}_3 > 90\%$ over $98.3\%$ of the sampled $(D,\eta)$ grid, while $f(\mathcal{R}_3>0.95)=0.2167$ in this setup. The interpolation profile is anchored by the benchmark convergence set (Table~\ref{tab:chi_loc}) and the auxiliary full-scan support points (Table~\ref{tab:chi_loc_full}) in Appendix~\ref{app:chi_loc}. This still supports a dominant three-layer sector, and the global map continues to use a simplified surrogate kinetic operator with only the mixing-profile input imported from the action-derived extraction.
...
To propagate finite-$N$ surrogate uncertainty into map-level systematics, we also perform a one-at-a-time local-parameter robustness scan around the baseline closure values: $c_{\rm eff}\in[0.45,0.55]$, $\nu\in[4.80,5.20]$, and $p_B\in[0.28,0.32]$ (with the same $60\times 60$ map, same $\chi_{LR}(D)$ interpolation, and all other settings fixed). Results are summarized in Table~\ref{tab:core_param_robustness} from \texttt{paper/core\_param\_robustness.csv}. Across this local window, the key fractions remain unchanged: $f(\mathcal{R}_3>0.90)=0.9833$, $f(\chi_{\mu\mu}^2<4)=0.0833$, and $f(N_{\rm win}>3)=0$.
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.9833/0.9833/0.9833$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.9833/0.9833/0.9833$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9833/0.9833/0.9833$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
with best grid point $\chi^2\simeq 5.10\times10^{-2}$ at $(D,\eta)\approx(9.69,0.20)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.
...
\end{enumerate}
At the same time, the present implementation keeps two EFT-level surrogates explicit: the Cardy-normalized \texttt{fp\_2d\_full} $g_N$ closure and the diagonal layer assignment in the $H\to\mu\mu$ map of Eq.~\eqref{eq:mu_pred}.
...
\begin{equation}
f(\mathcal{R}_3>0.90)=0.983,\quad
f(\mathcal{R}_3>0.95)=0.217,\quad
f(\chi^2_{\mu\mu}<4)=0.096,
\end{equation}
with the same best-fit point $(D,\eta)\approx(11.05,3.16)$. Thus, at current scan resolution, profile-interpolation uncertainty is subdominant to other model-systematics.
...
\hline
Linear interpolation & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
Exponential fit      & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
Linear $\times 0.8$  & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
Linear $\times 1.2$  & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
Exp-fit $\times 0.8$ & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
Exp-fit $\times 1.2$ & $0.983$ & $0.217$ & $0.083$ & $(9.69,\;0.20)$ \\
\hline\hline
\end{tabular}
\end{table*}
...
\end{equation}
As in the 1D candidate check, this benchmark is reported as a replacement candidate. To test map-level impact, we connected both candidates to the main scan through \texttt{g\_mode} choices (\texttt{fp\_2d\_full}, \texttt{cardy}, \texttt{fp\_1d}, \texttt{fp\_2d}) and the reproducible script \texttt{scan\_gn\_profile\_impact.py}. Results in Table~\ref{tab:gn_profile_impact} show that the main conclusion is retained: relative to baseline \texttt{fp\_2d\_full}, the largest absolute shift is $0.0167$ in $f(\mathcal{R}_3>0.90)$ (for \texttt{fp\_2d}), while high-$N$ runaway remains controlled with $f(N_{\rm win}>3)=0$ in all tested modes.
...
\hline
baseline fp\_2d\_full & 0.983 & 0.0833 & 0.000 & 0.0596 \\
legacy Cardy reference & 0.933 & 0.0833 & 0.000 & 0.0424 \\
$g_N$ fp\_1d profile & 0.933 & 0.0833 & 0.000 & 0.0437 \\
$g_N$ fp\_2d profile & 0.933 & 0.0833 & 0.000 & 0.0444 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.983 & 0.0833 & 0.000 & 0.059601 \\
baseline fp\_2d\_full & 30 & 0.983 & 0.0833 & 0.000 & 0.059601 \\
baseline fp\_2d\_full & 40 & 0.983 & 0.0833 & 0.000 & 0.059601 \\
$g_N$ fp\_2d profile & 20 & 0.933 & 0.0833 & 0.000 & 0.044400 \\
$g_N$ fp\_2d profile & 30 & 0.933 & 0.0833 & 0.000 & 0.044400 \\
$g_N$ fp\_2d profile & 40 & 0.933 & 0.0833 & 0.000 & 0.044400 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 0.983 & 0.0833 & 0.000 & 0.000 \\
legacy Cardy reference & 0.933 & 0.0833 & 0.000 & -0.050 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline (\texttt{fp\_2d\_full + localized}) & 0.983 & 0.0000 & 0.0833 & 0.0000 \\
legacy reference (\texttt{cardy + localized}) & 0.933 & -0.0500 & 0.0833 & 0.0000 \\
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.983 & 0.0000 & 0.0833 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $\eta_{\rm eff}=\eta$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
baseline $(A_1,A_2)=(1,1)$ & 0.9833 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$(A_1,A_2)=(\tilde A_1,\tilde A_2)$ & 0.9322 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
