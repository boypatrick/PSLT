...
(i) a \textbf{Micro-degeneracy module} ($g_N$) implemented in baseline \texttt{fp\_2d\_full} mode (bounded microcanonical low-$N$ window + energy-gap tail prescription, Cardy-normalized at $N=3$);
...
$g_N$ phase-space full-tail mode (\texttt{fp\_2d\_full}) with Cardy normalization anchor & Baseline mode + legacy Cardy reference retained for comparator audits & Yes & Replace Cardy anchor normalization with fully action-derived normalization if map-level stability is retained \\
...
& $c_{\rm eff},\nu,\kappa_g$ & $(0.5,5.0,0.03)$ & Cardy anchor parameters (legacy reference and normalization scale) \\
\midrule
...
\begin{enumerate}
    ...
    \item \textbf{Stability without ad-hoc cutoffs}: The infinite tower of layers becomes numerically and physically stable once micro-degeneracy is regulated at high $N$ [Eq.~\eqref{eq:gN_reg}] and the observable sector uses overlap-defined visibility profiles $B_N(D)$ with explicit high-$N$ saturation.
...
\end{enumerate}
At the same time, the present implementation keeps two EFT-level approximations explicit: the Cardy-anchored normalization inside \texttt{fp\_2d\_full} for $g_N$, and the diagonal layer assignment in the $H\to\mu\mu$ map of Eq.~\eqref{eq:mu_pred}.
...
\begin{equation}
g_N^{\rm(full)}(D)=g_3^{\rm(cardy)}\hat g_N(D),\quad
