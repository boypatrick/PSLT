...
$\chi_N^{(LR)}(D)$ localized channel & Stabilized localized-direct full-scan grid profile (\texttt{Dgrid60}) from 2D fine extraction + scan-level profile injection, released through \texttt{full\_direct\_map} mode (Appendix~\ref{app:chi_loc}; D60 + D21/E41 audits) & Yes (primary: \texttt{full\_direct}) & Promote to full per-$(D,\eta,N)$ localized direct projection (no profile injection) \\
