...
We verify the closure against two benchmarks: (1) internal stability of the three-generation hierarchy (Generation Ratio $> 90\%$ over $93.2\%$ of the sampled $(D,\eta)$ grid), and (2) an EFT-diagonal Wilson-coefficient compatibility test with the ATLAS Run-3 $H\to\mu\mu$ signal strength ($\mu_{\mu\mu}^{\rm obs}=1.4\pm0.4$).
The EFT-diagonal mapping yields a restricted acceptance band (about $8.3\%$ of the scanned grid for $\chi^2<4$), providing a sharper falsifiable constraint on $(D,\eta)$.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Verification}: We demonstrate stable three-generation dominance at $\mathcal{R}_3>90\%$ and a constrained EFT-diagonal-compatible band for LHC $H\to\mu\mu$.
\end{enumerate}
...
$H\to\mu\mu$ mapping [Eq.~\eqref{eq:mu_pred}] & EFT-diagonal Wilson map (scan-level) & Yes & Extend to full operator-basis EFT matching (including off-diagonal/operator-mixing terms) \\
\bottomrule
\end{tabular}
\end{table*}
...
\hline
$c_{\rm eff}$ & $0.45/0.50/0.55$ & $0.9322/0.9322/0.9322$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$\nu$ & $4.80/5.00/5.20$ & $0.9322/0.9322/0.9322$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
$p_B$ & $0.28/0.30/0.32$ & $0.9322/0.9322/0.9322$ & $0.0833/0.0833/0.0833$ & $\Delta f_{\mathcal{R}_3}=0,\ \Delta f_{\chi^2}=0$ \\
\hline\hline
\end{tabular}
\end{table}

\subsection{H to mumu Signal Strength (EFT-Diagonal Wilson Mapping)}
...
Within the PSLT closure, we construct a scan-level EFT-diagonal Wilson map by combining kinetic occupancy with overlap-extracted raw couplings:
\begin{equation}
...
\end{equation}
\begin{equation}
c_{\mu\mu}(D,\eta)=y^{\rm eff,raw}_{2}(D)\,P_2^{\rm(kin)}(D,\eta),\qquad
\mu_{\mu\mu}^{\rm pred}(D,\eta)=\left|\frac{c_{\mu\mu}(D,\eta)}{c_{\mu\mu}(D_0,\eta_0)}\right|^2.
\label{eq:mu_pred}
\end{equation}
Here $(D_0,\eta_0)=(10,1)$ is a fixed reference point. Equation~\eqref{eq:mu_pred} is an EFT-diagonal observable map (implemented in \texttt{pslt\_lib.py} with \texttt{hll\_observable\_mode=eft\_wilson\_diag}), not yet a full UV-to-EFT matching with operator mixing.
\begin{equation}
...
\end{equation}
Figure~\ref{fig:hmumu} shows the \textbf{EFT-diagonal acceptance region} in two equivalent languages,
\begin{equation}
...
\begin{equation}
f(\chi^2<4)=f\!\left(p_{\rm local}>4.55\times10^{-2}\right)=0.0833,
\end{equation}
with best grid point $\chi^2\simeq 5.10\times10^{-2}$ at $(D,\eta)\approx(9.69,0.59)$. All phase-diagram claims are quoted only after satisfying the quantitative convergence criteria in Appendix~\ref{app:Veff}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.48\linewidth]{hmumu_exclusion.png}
    \includegraphics[width=0.48\linewidth]{hmumu_signal_strength.png}
    \caption{Compatibility with $H\to\mu\mu$ under the EFT-diagonal Wilson map. Left: $\chi^2$ map showing a restricted illustrative accepted band ($\chi^2<4$, equivalently $p_{\rm local}>4.55\times10^{-2}$ for dof$=1$). Right: predicted $\mu_{\mu\mu}^{\rm pred}$.}
    \label{fig:hmumu}
\end{figure}

\subsection{EFT-Diagonal Extension to H to ee and H to tautau}
Using the same map-level EFT-diagonal definition, we additionally evaluate lepton-channel signal strengths by layer assignment $N=1\rightarrow ee$, $N=2\rightarrow \mu\mu$, and $N=3\rightarrow \tau\tau$:
\begin{equation}
\mu_{\ell\ell}^{\rm pred}(D,\eta)=\left|\frac{c_{\ell\ell}(D,\eta)}{c_{\ell\ell}(D_0,\eta_0)}\right|^2,\qquad
c_{\ell\ell}(D,\eta)=y^{\rm eff,raw}_{N}(D)\,P_N^{\rm(kin)}(D,\eta).
\end{equation}
...
\begin{figure}[htbp]
    ...
    \caption{PSLT EFT-diagonal extension from $H\to\mu\mu$ to three lepton channels on the same scan grid: $H\to ee$ (left), $H\to\mu\mu$ (middle), and $H\to\tau\tau$ (right). Each panel shows $\mu_{\ell\ell}^{\rm pred}=|c_{\ell\ell}/c_{\ell\ell}^{\rm ref}|^2$ with the same reference point $(D_0,\eta_0)=(10,1)$.}
    \label{fig:hll_proxy}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{Falsifiability}: The closure yields concrete predictions for the winner phase diagram. With D-interpolated $\chi_N^{(LR)}$, the $H\to\mu\mu$ illustrative EFT-diagonal acceptance band ($\chi^2<4 \Leftrightarrow p_{\rm local}>4.55\times10^{-2}$, dof$=1$) covers about $8.3\%$ of the scan and defines a falsifiable target region in $(D,\eta)$.
\end{enumerate}
At the same time, the present implementation keeps two EFT-level approximations explicit: profile interpolation in the scan-level $g_N(D)$/$\chi_N^{(LR)}(D)$ transfer chain, and the diagonal layer assignment in the $H\to\mu\mu$ map of Eq.~\eqref{eq:mu_pred}.
...
\begin{enumerate}
    \item \textbf{Observable mapping}: the $H\to\mu\mu$ comparison now uses a scan-level EFT-diagonal coefficient map $c_{\mu\mu}=y^{\rm eff,raw}_2P_2^{\rm(kin)}$, but a full derivation from the EYMH action (including operator mixing and Wilson matching) is still pending.
...
\hline
Global maps and EFT-diagonal observable plots & \texttt{generate\_plots.py}; \texttt{scan\_hll\_signal\_strengths.py} & \texttt{output/three\_generation\_*.png}; \texttt{output/hmumu\_*.png}; \texttt{output/hll\_signal\_strength/*} \\
...
f(\chi^2_{\mu\mu}<4)=0.0833,
\end{equation}
with the same best-fit point $(D,\eta)\approx(9.69,0.59)$. Thus, at current scan resolution, profile-interpolation uncertainty is subdominant to other model-systematics.
...
\hline
Linear interpolation & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
Exponential fit      & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
Linear $\times 0.8$  & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
Linear $\times 1.2$  & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
Exp-fit $\times 0.8$ & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
Exp-fit $\times 1.2$ & $0.932$ & $0.217$ & $0.083$ & $(9.69,\;0.59)$ \\
\hline\hline
\end{tabular}
\end{table*}

As a stronger stress test, we further rescaled the localized profile by large factors and tracked boundary movement directly on the $(D,\eta)$ grid. Figure~\ref{fig:chi_scale_stress} shows that the user-requested cases $\times0.5$ and $\times2$ remain exactly coincident with baseline boundaries at this scan resolution. In the present tested range (up to $\times10^6$), no visible boundary drift is detected for either the $H\to\mu\mu$ mask or the $\mathcal{R}_3>0.90$ mask. All boundary-coincidence statements in this paragraph refer to the present $60\times60$ scan resolution.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\linewidth]{chi_scale_stress_test.png}
    \caption{Strong $\chi$-amplitude stress test on the global map. Left: overlay of $\chi^2_{\mu\mu}\leq4$ boundary for selected scales, including the requested $\times0.5$ and $\times2$. Right: changed-cell fractions versus scale (relative to baseline $\times1$), showing no detected movement in the tested range. Source data file: \texttt{output/chi\_fp\_2d/chi\_scale\_stress\_test.csv}.}
...
\hline
baseline fp\_2d\_full & 0.932 & 0.0833 & 0.000 & 0.1214 \\
legacy Cardy reference & 0.882 & 0.0833 & 0.000 & 0.1070 \\
$g_N$ fp\_1d profile & 0.867 & 0.0833 & 0.000 & 0.1082 \\
$g_N$ fp\_2d profile & 0.865 & 0.0833 & 0.000 & 0.1088 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
baseline fp\_2d\_full & 20 & 0.932 & 0.0833 & 0.000 & 0.121385 \\
baseline fp\_2d\_full & 30 & 0.932 & 0.0833 & 0.000 & 0.121385 \\
baseline fp\_2d\_full & 40 & 0.932 & 0.0833 & 0.000 & 0.121385 \\
$g_N$ fp\_2d profile & 20 & 0.865 & 0.0833 & 0.000 & 0.108846 \\
$g_N$ fp\_2d profile & 30 & 0.865 & 0.0833 & 0.000 & 0.108846 \\
$g_N$ fp\_2d profile & 40 & 0.865 & 0.0833 & 0.000 & 0.108846 \\
\hline\hline
\end{tabular}
\end{table}
...
\end{equation}
followed by the same per-step finite-volume clipping used in \texttt{pslt\_lib.py}. In the present baseline run, we use $(\alpha,\beta,s)=(0.8,1.1,0.0)$. Results are summarized in Table~\ref{tab:gn_baseline_replacement} (source CSV: \texttt{paper/gn\_baseline\_replacement.csv}). Relative to baseline \texttt{fp\_2d\_full}, the legacy Cardy reference lowers $f(\mathcal{R}_3>0.90)$ by $0.0503$ (from $0.932$ to $0.882$), leaves $f(\chi^2_{\mu\mu}<4)$ unchanged at this resolution, and still yields $f(N_{\rm win}>3)=0$. Therefore, the map-level main conclusion is retained under this baseline switch.
...
\hline
baseline fp\_2d\_full & 0.932 & 0.0833 & 0.000 & 0.000 \\
legacy Cardy reference & 0.882 & 0.0833 & 0.000 & -0.0503 \\
\hline\hline
\end{tabular}
\end{table}
...
Across all sensitivity cases in Table~\ref{tab:chi_open_sensitivity}, the core scan summaries remain unchanged on this grid: $f(\mathcal{R}_3>0.90)=0.932$, $f(\mathcal{R}_3>0.95)=0.217$, $f(\chi^2_{\mu\mu}<4)=0.0833$, and $f(N_{\rm win}>3)=0$. These values are model-dependent because the Lindblad structure is still an effective environment ansatz and not yet a microscopic EYMH bath derivation; therefore, \texttt{chi\_mode=open\_system} is treated as scan-ready diagnostic only and is not propagated into baseline reported figures.
...
\hline
baseline (\texttt{fp\_2d\_full + localized}) & 0.932 & 0.0000 & 0.0833 & 0.0000 \\
legacy reference (\texttt{cardy + localized}) & 0.882 & -0.0503 & 0.0833 & 0.0000 \\
$\chi$ migration (\texttt{fp\_2d\_full + open\_system}) & 0.932 & 0.0000 & 0.0833 & 0.0000 \\
\hline\hline
\end{tabular}
\end{table}
...
\hline
constant $t_{\rm coh}=1$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\pi/\Delta\omega_{12}(D)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\min(\pi/\Delta\omega_{12},10^4)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
\hline\hline
\end{tabular}
\end{table}

The dephasing candidate leaves both $\mathcal{R}_3$ and $H\to\mu\mu$ summary fractions unchanged on this grid under the EFT-diagonal mapping. Therefore, in this manuscript, Eq.~\eqref{eq:tcoh_dephasing} is reported as a first-principles candidate benchmark only and is not propagated into the baseline reported figures.
...
case & $f(\mathcal{R}_3>0.90)$ & $f(\mathcal{R}_3>0.95)$ & $f(\chi^2_{\mu\mu}<4)$ & best $\chi^2$ \\
\hline
reference $\eta_{\rm eff}=\eta$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm amp}(D)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\eta_{\rm eff}=\eta\,\eta_{\rm prob}(D)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\eta_{\rm eff}=\eta_{\rm amp}(D)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
$\eta_{\rm eff}=\eta_{\rm prob}(D)$ & 0.983 & 0.217 & 0.0833 & $5.10\times10^{-2}$ \\
\hline\hline
\end{tabular}
\end{table}

At current scan resolution, profile-scaled and fully-closed implementations are nearly degenerate with the reference map-level summaries in this diagnostic chain under the EFT-diagonal observable metric. Therefore, Eq.~\eqref{eq:eta_pref_candidates} is reported as a first-principles candidate benchmark and is not propagated into baseline reported figures.
...
At current scan resolution, replacing the legacy constant-$A_\ell$ reference by the action-derived profile induces non-negligible shifts in $\mathcal{R}_3$ coverage, while the EFT-diagonal $H\to\mu\mu$ acceptance area remains unchanged. In the updated global scans of this manuscript, Eq.~\eqref{eq:superrad_A_profile} is used in baseline mode, and $(A_1,A_2)=(1,1)$ is retained only as a comparator.
