...
def eig_1d(U: np.ndarray, z: np.ndarray, n_states: int = 3) -> Tuple[np.ndarray, List[np.ndarray]]:
    ...
    # Compute only the lowest n_states eigenpairs (reviewer-grade: avoid full O(N^2) spectrum)
    E, V = la.eigh_tridiagonal(main, off, select='i', select_range=(0, n_states-1))
...
def scan_D(
    ...
    for D in D_list:
        ...
        for n, E in enumerate(Evals, start=1):
            ...
            # Turning points: compute regardless of stability (useful for diagnostics)
            tp = turning_points_near_zero(U, z, float(E))
            has_tp = (tp is not None)

            # Diagnostic action S_z: compute for bound states if either
            #   (i) stable bound, or (ii) allow_unstable_S=True (diagnostic mode).
            if has_bound and has_tp and (has_stable_bound or allow_unstable_S):
                S_diag, _tp2 = wkb_action_central(U, z, float(E))
            else:
                S_diag = np.nan

            # Physical action: only meaningful for stable bound (omega^2>0)
            S_phys = float(S_diag) if (has_stable_bound and np.isfinite(S_diag)) else np.nan

            is_diag_tunnel = bool(has_bound and has_tp and np.isfinite(S_diag) and (S_diag > 0.0))
            is_phys_tunnel = bool(has_stable_bound and has_tp and np.isfinite(S_phys) and (S_phys > 0.0))
            ...
            rows.append({
                ...
                "S_z": float(S_diag) if np.isfinite(S_diag) else np.nan,
                "S_z_physical": float(S_phys) if np.isfinite(S_phys) else np.nan,
                ...
                "is_diagnostic_tunnel": bool(is_diag_tunnel),
...
def main():
    ...
    ap.add_argument("--rep_prefix", type=str, default="rep_axis1d_v3p1")

    ap.add_argument("--allow_unstable_S", action="store_true",
                    help="If set, compute diagnostic WKB action S_z for bound states even when omega2<=0. "
                         "Physical tunneling still requires has_stable_bound.")
    ap.add_argument("--no_allow_unstable_S", action="store_true",
                    help="Force-disable allow_unstable_S even for diagnostic_deepwell preset.")
    ...
    ap.add_argument("--conv_out", type=str, default="convergence_axis1d_v3p1.csv")
    ...
    # Diagnostic/physical split for WKB action:
    #   - diagnostic_deepwell: allow_unstable_S defaults to True (so S_z is populated)
    #   - physical_gap: allow_unstable_S defaults to False
    allow_unstable_S = (args.preset == "diagnostic_deepwell")
    if args.allow_unstable_S:
        allow_unstable_S = True
    if args.no_allow_unstable_S:
        allow_unstable_S = False

    if args.out is None:
        out = f"scan_axis1d_v3p1_{args.preset}.csv"
    else:
        out = args.out
    ...
    df, rep_data = scan_D(D_list, a, eps, m0, xi, zmax, Nz, nstates, reps, allow_unstable_S=allow_unstable_S)
